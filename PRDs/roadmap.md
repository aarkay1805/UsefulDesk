...
Built: attendance + plan visit limits / session packs, with separate Name and Plan register columns and All-members-parity Plan header filtering · **account-wide immutable Member IDs with quick front-desk ID check-in and future multi-branch/biometric-safe identity** · a persisted freezable Name column in All Members · at-risk members via churn-risk flags · dormant recovery through Renewals · full owner reporting · **the Finance Overview foundation: a calendar-month bird's-eye view with revenue comparison, next-month renewal projection, day/weekly income cash flow, invoice health, collection mix, recent transactions, and admin-only CSV export; operational dues, payment entry, reminders, server-paged payment history, and complete filtered export remain in Members → Payments**.

Left: trainer accountability · **Finance master section** (`PRDs/finance_master_section.md`) later tabs and supporting domains: minimal expense tracking first to unlock real Expenses and Profit, then the account-wide invoice master and analytical Payments ledger; payment-reference/Member-ID search and the failed-AutoPay recovery loop stay with Members → Payments; GST-ready documents remain an optional later phase after immutable invoice identity and compliance validation.
