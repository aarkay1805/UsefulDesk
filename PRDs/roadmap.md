...
Engineering foundation: **the native UsefulDesk Agent app now has a separate
`@usefuldesk/mobile` Expo SDK 57 npm workspace at `apps/mobile`; the Next.js
web app remains at the repository root, and root mobile scripts delegate only
to the native workspace. The temporary Expo Router root screen is intentionally
minimal while subsequent mobile work establishes the authenticated product.**
