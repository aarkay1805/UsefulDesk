...
detail and inbox contact panel icon row. It still opens the existing
confirmation dialog; the pinned footer action is gone.
...
send behavior remains page-owned. Same-conversation reconnect, foreground, and
manual catch-up requests retain rendered bubbles and loaded-history cursors while
...
Engineering maintenance: **the complete Members import wizard is distilled and polished: compact upload, confirmation, and result layouts; shared mapping table with inline duplicate-field guidance, a definite bounded dialog height for reliable vertical scrolling, a mapped/skipped section heading, and the shared date-order Select; accessible current-step announcements; concise draft status and pending actions; Fee/Paid/Balance on phone review rows; a clear ready state; excluded-row export in the Excluded view; and invoice/source details on demand. Resolve issues splits the full modal height: wizard heading, steps, worksheet, and draft navigation on the left; member header, independently scrolling corrections, and pinned resolution actions on the right. Phones switch between the complete list and inspector. Review keeps name/phone and navigation together and adapts the pinned footer to available actions: a lone edit fills its row, manual exclusions get one Include action, and automatic exclusions have no action footer. Repeated worksheet count strips are removed, pagination appears only for multiple pages, and steps 2–4 put draft status, Start fresh, and recovery actions at the leading edge of the footer, with Import rules and the current primary action trailing. Upload groups file details, draft status, Change file, and Start fresh in one responsive file area with visible recovery; its footer pairs Map manually with Analyze file, while Sample CSV, file requirements, privacy, and import rules share one expandable section. Back is a shared chevron icon button beside the heading on steps 2–4. Close aligns with the header controls, including the active inspector header in Resolve issues, without changing the shared dialog master. Upload removes the duplicate Save & close/Cancel action; the header close control still flushes pending changes and stays open on save failure. Import notices expand on demand. Confirmation counts unique customers, and failed/empty results point to the receipt without claiming success. Manual mapping, grouped corrections and exclusion scopes, draft recovery, and import transactions remain available; shared UI masters and schema are unchanged. Locally verified; no live import.**
...
Engineering maintenance: **Members import hover feedback now follows the action: mapping rows and static headers stay passive, clickable review rows keep their highlight, and shared buttons, fields, picker triggers, and checkboxes use canonical hover tokens with disabled/read-only and validation/focus guards. Shared accordion triggers use rounded neutral hover fills without underlines, preserving label alignment and responsive container gutters, including the Reports and Finance disclosures. The shared fixes apply across their consumers; other tables retain their existing default behavior.**

Engineering maintenance: **the complete Members import wizard now clarifies action scope, draft versus committed changes, rows versus members, recorded payments, and exclusion reasons. Closing every issue group retains the 400px desktop resolution panel with simple instructions for choosing a row and finding its fixes; reopening a group restores its correction controls. Unreadable drafts expose reload/start-fresh recovery, all-excluded imports lead back to their excluded rows, and results offer an Import report with clear partial/failure outcomes. Existing consent, import transactions, report format, and shared UI masters are unchanged.**
