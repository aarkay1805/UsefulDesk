...
Built: attendance + plan visit limits / session packs, with separate Name and Plan register columns and All-members-parity Plan header filtering · **account-wide immutable Member IDs with quick front-desk ID check-in and future multi-branch/biometric-safe identity** · a persisted freezable Name column in All Members · at-risk members via churn-risk flags · dormant recovery through Renewals · full owner reporting · **the Finance Phase A foundation: an actionable Overview and Collections home, existing payment/due flows consolidated out of Members, URL-compatible deep links, server-paged payment history, complete filtered CSV export, and staff/date/method/source/status filters**.

Left: trainer accountability · **Finance master section** (`PRDs/finance_master_section.md`) hardening and later phases: payment-reference/Member-ID search plus filtered amount totals, the failed-AutoPay recovery loop, an account-wide invoice master, then minimal expense tracking and net cash; GST-ready documents remain an optional later phase after immutable invoice identity and compliance validation.
