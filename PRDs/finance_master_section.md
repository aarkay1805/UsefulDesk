...
6. How much revenue came from joins, renewals, and recovered dues—and did this month's paid social cohort return more joining revenue than its Marketing spend?
...
- Revenue breakdown is the same selected-month, account-timezone paid ledger grouped by immutable payment purpose: New memberships, Renewals, Due payments recovered, and Other. Each stream is an expandable summary with payment count, total-revenue share, and revenue; its disclosure groups the same paid rows by plan with payment count, within-stream share, and revenue. Other appears only when non-zero so the rows always reconcile with Revenue without mislabelling legacy or mid-cycle plan-change collections.
