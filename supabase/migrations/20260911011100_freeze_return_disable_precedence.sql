-- A disabled lifecycle is terminal even when its previously planned date is
-- still in the future. Keep the no-send configuration boundary ahead of the
-- date gate so stale jobs cannot be retried indefinitely after disablement.
...
BEGIN
  ...
  IF v_job.effective_due_on > v_today THEN RETURN 'not_eligible'; END IF;
  ...
  IF v_owner IS NULL THEN
    UPDATE public.lifecycle_reminder_jobs SET escalation_state = 'stopped', escalated_at = NOW() WHERE id = v_job.id;
    RETURN 'stopped';
  END IF;
  IF NOT EXISTS (
...
REVOKE ALL ON FUNCTION public.create_freeze_return_follow_up(UUID) FROM PUBLIC, anon, authenticated;
