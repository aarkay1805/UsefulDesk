-- Payment collection takes the one-per-contact daily claim before a missed-visit check-in.
...
BEGIN
  ...
  v_candidate_priority := CASE v_job.kind WHEN 'installment_overdue' THEN 4 WHEN 'invoice_overdue' THEN 3 WHEN 'invoice_due' THEN 2 WHEN 'attendance_absence' THEN 1 ELSE 5 END;
  IF EXISTS (SELECT 1 FROM public.lifecycle_reminder_jobs other WHERE other.account_id=v_job.account_id AND other.contact_id=v_job.contact_id AND other.id<>v_job.id AND other.state IN ('queued','leased','deferred','blocked') AND other.next_attempt_at<=NOW() AND (CASE other.kind WHEN 'installment_overdue' THEN 4 WHEN 'invoice_overdue' THEN 3 WHEN 'invoice_due' THEN 2 WHEN 'attendance_absence' THEN 1 ELSE 5 END)>v_candidate_priority) THEN RETURN 'deferred'; END IF;
...
REVOKE ALL ON FUNCTION public.reserve_lifecycle_reminder_daily_claim(UUID, TEXT, INTEGER, DATE)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.reserve_lifecycle_reminder_daily_claim(UUID, TEXT, INTEGER, DATE)
  TO service_role;
