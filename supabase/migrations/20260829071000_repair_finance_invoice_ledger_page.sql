-- Forward repair: LEAST and GREATEST are SQL expressions and cannot be
-- schema-qualified as pg_catalog functions. The original migration applied
-- before its first authenticated invocation exposed the resolution error.
  ...
  export_line_details AS (
    ...
    FROM (
      SELECT
        ...
        pg_catalog.min(source.id) AS id
