-- Add operational Assigned-to and Trainer facets to the one All-members
-- snapshot. IDs remain caller-supplied filters only: the SECURITY INVOKER view
-- and contact/profile/trainer RLS continue to enforce the selected account.

DROP FUNCTION IF EXISTS public.member_customer_directory_page(
