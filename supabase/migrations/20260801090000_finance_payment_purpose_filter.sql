-- Let Finance Overview deep-link each immutable revenue source into the
-- account-wide Payments ledger without filtering a paginated result client-side.
...
AS $$
  WITH base AS (
    ...
    WHERE public.is_account_member(payment.account_id)
      ...
      AND (
        COALESCE(CARDINALITY(p_purposes), 0) = 0
        OR payment.payment_purpose = ANY(p_purposes)
      )
      AND (
...
COMMENT ON FUNCTION public.finance_payment_ledger(
  ...
  'Tenant-scoped analytical payment ledger page with exact filtered totals, revenue-purpose filtering, and quick-view facets.';
