-- `renew_membership_transaction` is a real authoritative source but its
-- immutable payment-purpose projection can be `due`. The durable operation
-- key is the factual renewal proof, so do not weaken it with that projection.
