-- Google OAuth normally provides both `full_name` and `name`, but the
-- normalized provider payload is not a contract we should make tenant
-- provisioning depend on. Accept either display-name field while preserving
-- the atomic invariant: no Auth user survives without a complete tenant.
