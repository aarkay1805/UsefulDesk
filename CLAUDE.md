...
- **Migrations:** sequential + idempotent in `supabase/migrations/` (enum in `DO $$ IF NOT EXISTS` block, `CREATE TABLE/INDEX IF NOT EXISTS`, drop-then-create policies, reuse `update_updated_at_column()`). Latest is `046`. The live Supabase project is `UsefulDesk` (`fwqthstqrkrwtaehefks`) — new migrations are applied to it via the Supabase MCP `apply_migration` (or `supabase db push`); after applying, verify with a `pg_policies`/schema query.
