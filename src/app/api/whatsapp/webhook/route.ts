...
async function handleStatusUpdate(
  ...
  if (!Number.isFinite(statusAt.getTime())) {
    console.warn('[webhook] ignored status with invalid timestamp', status.id);
    return;
  }

  // The signed payload's phone_number_id is the tenant boundary. The
  // service-only RPC applies the message + broadcast mirrors in one
  ...
  // actually advanced. Duplicate, unknown, cross-tenant, and regressive
  // callbacks return no row from the RPC.
  for (const row of (changedMessages ?? []) as Array<{
    account_id: string;
