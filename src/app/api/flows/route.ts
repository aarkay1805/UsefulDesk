 ...
 *
 * GET is available to every authenticated account member. POST requires
 * operational access (agent+) because it writes through the service role.
 * The previous per-account beta gate was removed when Flows went to soft-GA;
 * the UI still shows a "Beta" label so users know the surface is young.
 */
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!canEditAuthoredContent(ctx.role, ctx.userId, ctx.userId)) {
      throw new ForbiddenError('You cannot create flows');
    }
  } catch (err) {
