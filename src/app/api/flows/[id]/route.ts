 ...
 *                          runs, events).
 *
 * GET requires current-account membership. PUT and DELETE require
 * operational access (agent+) before any service-role work. Flows is in
 * soft-GA — the beta gate that previously 404'd non-beta accounts is gone;
 * the "Beta" label in the UI is the only remaining signal.
 */
...
> {
  ...
  // RLS scopes this to the caller's current account. A flow in another
  // tenant returns null (404 below).
