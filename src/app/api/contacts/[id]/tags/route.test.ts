...
beforeEach(() => {
  mocks.requireOperationalAccess.mockReset();
  mocks.add.mockReset();
...
describe('/api/contacts/[id]/tags', () => {
  it('requires operational access and dispatches a newly-added tag', async () => {
    mocks.add.mockResolvedValue({ added: true, dispatched: true });

    const response = await POST(request('POST', { tag_id: 'tag-1' }), params);

    expect(response.status).toBe(200);
    expect(mocks.requireOperationalAccess).toHaveBeenCalledOnce();
