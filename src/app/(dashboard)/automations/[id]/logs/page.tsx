              ...
              >
                ...
                {isOpen && (
                  <div className="border-t border-border px-4 py-3">
                    {log.error_message && (
                      <p className="mb-3 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-red-foreground">
                        {log.error_message}
                      </p>
                    )}
...
function StatusBadge({ status }: { status: AutomationLog["status"] }) {
  const classes =
    status === "success"
      ...
      : status === "partial"
      ? "border-amber-500/30 bg-amber-500/10 text-amber-foreground"
    ...
    <li className="flex items-start gap-2 text-xs">
      <span
        className={cn(
          ...
          ok ? "bg-primary/20 text-primary-text" : "bg-red-500/20 text-red-foreground",
        )}
        aria-hidden
      >
