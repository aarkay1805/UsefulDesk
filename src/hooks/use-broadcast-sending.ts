 ...
 * A campaign is many API calls, so RATE_LIMITS.broadcast must carry this
 * per-batch pattern rather than treating one campaign as one request.
 */
  ...
  async function createAndSendBroadcast(
    ...
    try {
      ...
      for (let i = 0; i < recipients.length; i += SEND_BATCH_SIZE) {
        ...
        try {
          const res = await fetchBroadcastBatchWithRetry(
