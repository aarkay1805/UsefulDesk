...
export function ValidationPanel() {
  ...
  if (issues.length === 0) {
    ...
    return (
      <div className="flex items-center gap-2 rounded-lg border border-emerald-600/50 bg-background p-3 text-sm font-medium text-emerald-foreground">
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground">
        {errors.length > 0 ? (
          <CircleAlert className="h-4 w-4 text-red-foreground" />
        ) : (
          <CircleAlert className="h-4 w-4 text-amber-foreground" />
        )}
...
export function IssueLine({
  ...
  const tone =
    issue.severity === "error" ? "text-red-foreground" : "text-amber-foreground";
  const iconTone =
    issue.severity === "error" ? "text-red-foreground" : "text-amber-foreground";
  const body = (
    <>
