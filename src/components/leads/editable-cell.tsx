    ...
    return (
      ...
      >
        ...
        <span
          className={cn(
            INNER,
            'group-hover/cell:bg-muted/70 group-hover/cell:ring-border group-hover/cell:ring-1 group-hover/cell:ring-inset'
          )}
        >
          {display}
          ...
          {isMenuKind && (
            <ChevronDown className="text-muted-foreground ml-auto size-4 shrink-0 opacity-0 transition-opacity group-hover/cell:opacity-100" />
          )}
        </span>
      </button>
    );
          ...
          <DropdownMenuTrigger
            render={
              <button
                ...
                className={cn(
                  EDIT_INNER,
                  'justify-between disabled:opacity-60'
                )}
              />
            }
          >
          ...
          >
            {options?.map((o) => (
              <DropdownMenuItem
                ...
                className="text-popover-foreground focus:bg-muted flex items-center justify-between gap-3"
              >
                {optionLabel(o)}
                {o.value === value && (
  ...
  return (
    ...
    >
      {kind === 'phone' ? (
        ...
        <div className="bg-card ring-primary flex h-8 w-full min-w-0 items-center rounded-md ring-2 ring-inset">
          <span className="text-muted-foreground pointer-events-none shrink-0 pl-2.5 text-sm">
            {prefix}
          </span>
          <input
            {...inputProps}
            className="text-foreground h-full w-full min-w-0 rounded-md bg-transparent pr-13 pl-1.5 text-sm outline-none disabled:opacity-60"
          />
        </div>
      ) : (
        <input
          ...
          className="bg-card text-foreground ring-primary h-8 w-full rounded-md pr-13 pl-2.5 text-sm ring-2 outline-none ring-inset disabled:opacity-60"
        />
      )}
      <InlineEditActions
        saving={saving}
