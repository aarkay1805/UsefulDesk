  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        ...
        "border-input-border not-disabled:not-data-disabled:not-data-readonly:not-aria-invalid:not-focus-visible:hover:border-border-hover focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 data-placeholder:text-muted-foreground dark:bg-input/30 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 flex w-fit items-center justify-between gap-1.5 rounded-lg border bg-transparent py-2 pr-2 pl-2.5 text-sm whitespace-nowrap transition-colors outline-none select-none focus-visible:ring-3 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:ring-3 data-[size=default]:h-8 data-[size=sm]:h-7 data-[size=sm]:rounded-[min(var(--radius-md),10px)] *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
    ...
    <SelectPrimitive.Portal>
      ...
      >
        ...
        >
          ...
          {/* p-1 lives on the list (like DropdownMenuContent's popup) so
              items get consistent inset whether or not they're wrapped in a
              SelectGroup — a bare-item select must not render flush to the
              popup edge. Kept off the popup itself so the scroll arrows span
              full width. */}
