...
describe('TemplatePicker', () => {
  ...
  it('opens a ready-to-send review with membership values filled automatically', async () => {
    ...
    expect(
      await screen.findByText(
        'Hi Asha Rao, your Quarterly membership ends on 20 Sep 2026. Current renewal price: ₹3,999. Reply to FitZone Wellness Private Limited for help renewing.'
      )
    ).toBeTruthy();
    expect(
  ...
  it('uses the immutable invoice number in a payment-link preview', async () => {
    ...
    expect(
      await screen.findByText(
        'Hi Asha Rao, ₹2,700 is due for invoice INV-000042. This payment link expires on 20 Sep 2026, 6:00 pm. Pay FitZone Wellness Private Limited using the button below.'
      )
    ).toBeTruthy();
  });
  ...
  it('fills a service renewal when exactly one renewable service is available', async () => {
    ...
    expect(
      await screen.findByText(
        'Hi Asha Rao, your Personal training service ends on 20 Sep 2026. Current renewal price: ₹2,500. Reply to FitZone Wellness Private Limited for help renewing.'
      )
    ).toBeTruthy();
    expect(
