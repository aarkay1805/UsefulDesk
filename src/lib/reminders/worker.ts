...
import {
  selectPostExpiryMilestone,
  shouldEscalatePostExpiry,
} from './post-expiry';
import {
  processRetentionJob,
...
type CollectionCommitment = {
  ...
  invoice_id: string;
  contact_id: string;
  kind: 'promise_to_pay' | 'verification_hold' | 'dispute_hold';
  state: string;
  amount: number | null;
  promised_on: string | null;
  revision: number;
  created_at: string;
  payment_allocation_snapshot?: number;
};
...
async function needsAutoPayReconciliation(
  ...
  const membershipByPeriod = new Map(
    (periods ?? []).map((period) => [
      period.id as string,
      period.membership_id as string,
    ])
  );
  return requiresAutoPayReconciliation({
    ...
    lineMembershipIds: periodIds.map(
      (periodId) => membershipByPeriod.get(periodId) ?? null
    ),
  });
}
...
async function findOrCreateConversation(
  ...
  const { data: created, error } = await admin
    ...
    .single();
  if (error || !created)
    throw new Error(error?.message ?? 'conversation unavailable');
  return created.id as string;
}

export function postExpiryReplyStartAt(
  endDate: string,
  timeZone: string
): string {
...
async function currentPostExpirySubject(
  ...
  if (job.kind === 'membership_post_expiry') {
    const { data, error } = await admin
      .from('memberships')
      .select(
        'id, account_id, contact_id, end_date, status, collection_mode, fee_amount, contact:contacts(id, name, phone), plan:membership_plans(name, plan_type)'
      )
      .eq('account_id', job.account_id)
  ...
  if (
    !service ||
    service.contact_id !== job.contact_id ||
    service.end_date !== job.effective_due_on
  )
    return null;
  return isExpiredRenewalCandidate({
...
async function processPostExpiryJob({
  ...
  if (settingError || !rawSetting) {
    await finishJob(admin, job, 'blocked', {
      reason: { code: 'post_expiry_settings_unavailable' },
    });
    summary.blocked++;
    return;
  }
  const settings = postExpirySettings(
  ...
  if (
    ...
    await finishJob(admin, job, 'skipped', {
      reason: { code: 'post_expiry_no_longer_active' },
    });
    summary.skipped++;
    return;
  }
  ...
  try {
    subject = await currentPostExpirySubject(admin, job, today);
  } catch {
    await finishJob(admin, job, 'blocked', {
      reason: { code: 'post_expiry_subject_unavailable' },
    });
  ...
  if (!subject) {
    await finishJob(admin, job, 'skipped', {
      reason: { code: 'post_expiry_subject_changed' },
    });
    summary.skipped++;
    return;
  }
  try {
    ...
    await finishJob(admin, job, 'blocked', {
      reason: { code: 'reply_history_unavailable' },
    });
    summary.infrastructureFailures++;
    summary.blocked++;
    return;
  ...
  if (!latest || latest.key !== job.milestone_key) {
    await finishJob(admin, job, 'skipped', {
      reason: { code: 'superseded_or_expired_milestone' },
    });
    summary.skipped++;
    return;
  }
  ...
  const template =
    kind === 'membership_post_expiry'
      ? TEMPLATE_CONTRACTS.membership_post_expiry
      : TEMPLATE_CONTRACTS.service_post_expiry;
  ...
  const params =
    kind === 'membership_post_expiry'
      ? [
          (subject as MembershipExpiryCandidate).contact?.name?.trim() ||
            'there',
          (subject as MembershipExpiryCandidate).plan?.name || 'membership',
          fmt.date(job.effective_due_on),
          fmt.money(Number((subject as MembershipExpiryCandidate).fee_amount)),
          account.legalBusinessName!,
        ]
      : [
          (subject as ServiceExpiryCandidate).member_name?.trim() || 'there',
          (subject as ServiceExpiryCandidate).item_name_snapshot,
          fmt.date(job.effective_due_on),
          fmt.money(
            Number((subject as ServiceExpiryCandidate).current_renewal_price)
          ),
          account.legalBusinessName!,
        ];
  ...
  try {
    ...
    const { whatsapp_message_id } = await engineSendTemplate({
      beforeSend: async () => {
        const { data: finalSetting, error: finalSettingError } = await admin
          ...
          .eq('account_id', job.account_id)
          .maybeSingle();
        if (finalSettingError)
          throw new ReminderInfrastructureError(
            'post_expiry_settings_unavailable'
          );
        const final = finalSetting
          ? postExpirySettings(finalSetting as Record<string, unknown>, kind)
          : null;
        if (
          !final ||
          !final.enabled ||
          final.generation !== job.activation_generation
        )
          throw new ReminderNoLongerEligibleError(
            'post_expiry_no_longer_active'
          );
        const finalToday = todayInTz(locale.timeZone, new Date());
        if (
          !isWithinReminderSendWindow(
            hourInTz(locale.timeZone, new Date()),
            final.sendWindowStart,
            final.sendWindowEnd
          )
        )
          throw new ReminderSetupChangedError('send window changed');
        const finalSubject = await currentPostExpirySubject(
          admin,
          job,
          finalToday
        );
        if (
          !finalSubject ||
          (await hasCustomerReplySinceExpiry(admin, job, locale.timeZone))
        )
          throw new ReminderNoLongerEligibleError(
            'post_expiry_subject_changed_or_replied'
          );
        ...
        params[3] = fmt.money(
          Number(
            kind === 'membership_post_expiry'
              ? (finalSubject as MembershipExpiryCandidate).fee_amount
              : (finalSubject as ServiceExpiryCandidate).current_renewal_price
          )
        );
    ...
    if (
      shouldEscalatePostExpiry({
        milestoneKey: job.milestone_key,
        state: 'accepted',
        hasCustomerReply: false,
      })
    ) {
      const { data: escalation, error } = await admin.rpc(
        'escalate_post_expiry_reminder',
        { p_job_id: job.id }
      );
      if (error)
        summary.notes.push(
          `job ${job.id}: escalation unavailable — ${error.message}`
        );
      else
        summary.notes.push(
          `job ${job.id}: post-expiry escalation ${String(escalation)}`
        );
    }
  } catch (error) {
    if (error instanceof ReminderNoLongerEligibleError) {
      await finishJob(admin, job, 'skipped', {
        reason: { code: error.message },
      });
      summary.skipped++;
    } else if (error instanceof ReminderInfrastructureError) {
      await finishJob(admin, job, 'blocked', {
        reason: { code: error.message },
      });
      summary.infrastructureFailures++;
...
async function queuePostExpiryCandidates({
  ...
  const membershipSettings = postExpirySettings(
    rawSetting,
    'membership_post_expiry'
  );
  ...
  const { data: history, error: historyError } = await admin
    .from('lifecycle_reminder_jobs')
    .select(
      'membership_id, member_service_id, effective_due_on, activation_generation, milestone_key'
    )
  ...
  for (const row of history ?? []) {
    if (row.membership_id) {
      const key = postExpiryCycleKey(
        row.membership_id as string,
        row.effective_due_on as string,
        row.activation_generation as string
      );
      handledMembership.set(key, [
        ...(handledMembership.get(key) ?? []),
        row.milestone_key as string,
      ]);
    }
    if (row.member_service_id) {
      const key = postExpiryCycleKey(
        row.member_service_id as string,
        row.effective_due_on as string,
        row.activation_generation as string
      );
      handledService.set(key, [
        ...(handledService.get(key) ?? []),
        row.milestone_key as string,
      ]);
    }
  }
  if (
    ...
    membershipSettings.activatedOn &&
    membershipSettings.activatedAt
  ) {
    const { data, error } = await admin
      .from('memberships')
      .select(
    ...
    for (const member of (data ??
      []) as unknown as MembershipExpiryCandidate[]) {
      if (
        !isExpiredRenewalCandidate({
        ...
        continue;
      const cycleKey = postExpiryCycleKey(
        member.id,
        member.end_date,
        membershipSettings.generation
      );
      const milestone = selectPostExpiryMilestone({
        endDate: member.end_date,
        today,
        activatedOn: membershipSettings.activatedOn,
        handledKeys: handledMembership.get(cycleKey) ?? [],
        catchUpDays: membershipSettings.catchUpDays,
      });
        ...
        .upsert(
          {
            ...
            business_key: createReminderBusinessKey({
              kind: 'membership_post_expiry',
              invoiceId: null,
              subjectCycleId,
            ...
            activation_generation: membershipSettings.generation,
          },
      ...
      if (queueError)
        summary.notes.push(
          `membership ${member.id}: post-expiry queue failed — ${queueError.message}`
        );
  ...
  if (
    ...
    serviceSettings.activatedOn &&
    serviceSettings.activatedAt
  ) {
    const { data, error } = await admin
      .from('service_renewal_queue')
      .select(
        'id, account_id, contact_id, end_date, status, created_at, item_name_snapshot, current_renewal_price, item_is_active, option_is_active, member_name, phone'
      )
      .eq('account_id', account.id)
      .lt('end_date', today)
    ...
    for (const service of (data ?? []) as unknown as ServiceExpiryCandidate[]) {
      ...
      const cycleKey = postExpiryCycleKey(
        service.id,
        service.end_date,
        serviceSettings.generation
      );
      const milestone = selectPostExpiryMilestone({
        endDate: service.end_date,
        today,
        activatedOn: serviceSettings.activatedOn,
        handledKeys: handledService.get(cycleKey) ?? [],
        catchUpDays: serviceSettings.catchUpDays,
      });
      if (!milestone) continue;
        ...
        .upsert(
          {
            ...
            coordination_on: today,
            activation_generation: serviceSettings.generation,
          },
          { onConflict: 'account_id,business_key', ignoreDuplicates: true }
        )
        .select('id');
      if (queueError)
        summary.notes.push(
          `service ${service.id}: post-expiry queue failed — ${queueError.message}`
        );
      else if (created?.length) summary.queued++;
    }
  }
...
async function reconcilePostExpiryEscalations(
  ...
  if (error) {
    summary.infrastructureFailures++;
    summary.notes.push(
      `post-expiry escalation reconciliation unavailable: ${error.message}`
    );
    return;
  }
  for (const job of data ?? []) {
    const { data: result, error: escalationError } = await admin.rpc(
      'escalate_post_expiry_reminder',
      { p_job_id: job.id }
    );
    if (escalationError) {
      summary.infrastructureFailures++;
      summary.notes.push(
        `job ${job.id}: escalation unavailable — ${escalationError.message}`
      );
    } else {
      summary.notes.push(
        `job ${job.id}: post-expiry escalation ${String(result)}`
      );
    }
  }
}
...
async function queueCommitmentAndLinkCandidates({
  ...
  const [
    ...
    admin
      .from('lifecycle_reminder_jobs')
      .select(
        'collection_commitment_id, commitment_revision, payment_link_id, payment_link_revision, milestone_key, state'
      )
      .eq('account_id', account.id),
    promise.enabled && promise.activatedAt
      ? admin
          ...
          .select(
            'id, invoice_id, contact_id, kind, state, amount, promised_on, revision, created_at'
          )
          .eq('account_id', account.id)
          .eq('kind', 'promise_to_pay')
          .in('state', ['open', 'broken'])
          .gte('created_at', promise.activatedAt)
      : Promise.resolve({ data: [], error: null }),
    linkFollowUp.enabled && linkFollowUp.activatedAt
      ? admin
          .from('razorpay_payment_links')
          .select(
            'id, invoice_id, revision, expected_amount, short_url, expires_at, status, last_sent_at, last_whatsapp_message_id, invoice:invoices(contact_id)'
          )
          .eq('account_id', account.id)
          .eq('status', 'created')
          .not('last_sent_at', 'is', null)
          .gte('last_sent_at', linkFollowUp.activatedAt)
      : Promise.resolve({ data: [], error: null }),
  ]);
  if (historyError || commitmentsError || linksError)
    throw historyError ?? commitmentsError ?? linksError;
  for (const item of (commitments ?? []) as unknown as CollectionCommitment[]) {
    if (
      !promise.enabled ||
      !promise.activatedOn ||
      !item.promised_on ||
      item.promised_on < promise.activatedOn
    )
      continue;
    const reconciliation = await admin.rpc(
      'reconcile_invoice_collection_commitment',
      { p_id: item.id }
    );
    ...
    const handled = (history ?? [])
      .filter(
        (job) =>
          job.collection_commitment_id === item.id &&
          Number(job.commitment_revision) === item.revision &&
          ['accepted', 'delivered', 'ambiguous', 'skipped'].includes(
            job.state as string
          )
      )
      .map((job) => job.milestone_key as string);
    if (broken && !handled.includes('promise-broken')) {
        ...
        .upsert(
          {
            ...
            collection_commitment_id: item.id,
            commitment_revision: item.revision,
            kind: 'promise_to_pay',
            subject_cycle_id: subjectCycleId,
            milestone_key: 'promise-broken',
            business_key: createReminderBusinessKey({
              kind: 'promise_to_pay',
              invoiceId: item.invoice_id,
              subjectCycleId,
              milestoneKey: 'promise-broken',
            }),
            effective_due_on: item.promised_on,
            coordination_on: today,
            activation_generation: promise.generation,
          },
          { onConflict: 'account_id,business_key', ignoreDuplicates: true }
        )
        .select('id');
      if (error)
        summary.notes.push(
          `broken commitment ${item.id}: queue failed — ${error.message}`
        );
    ...
    const milestone = selectDueMilestone({
      anchorDate: item.promised_on,
      today,
      activatedOn: promise.activatedOn,
      milestones: [
        { key: 'promise-before-1', offsetDays: -1 },
        { key: 'promise-due', offsetDays: 0 },
      ],
      handledKeys: handled,
      catchUpDays: 1,
    });
    ...
    if (error)
      summary.notes.push(
        `commitment ${item.id}: queue failed — ${error.message}`
      );
    else if (data?.length) summary.queued++;
  }
  for (const link of (links ?? []) as unknown as PaymentLinkCandidate[]) {
    if (
      ...
      !linkFollowUp.activatedOn ||
      !link.last_sent_at ||
      !link.last_whatsapp_message_id ||
      !link.short_url ||
      !link.invoice?.contact_id
    )
      continue;
    if (new Date(link.expires_at) <= now) {
      const { data: escalation, error } = await admin.rpc(
        'escalate_expired_payment_link',
        { p_link_id: link.id }
      );
      if (error)
        summary.notes.push(
          `payment link ${link.id}: expiry action unavailable — ${error.message}`
        );
      else
        summary.notes.push(
          `payment link ${link.id}: expiry action ${String(escalation)}`
        );
      continue;
    }
    const sentOn = todayInTz(
      resolveAccountLocale(account).timeZone,
      new Date(link.last_sent_at)
    );
    if (sentOn < linkFollowUp.activatedOn) continue;
    const handled = (history ?? [])
      .filter(
        (job) =>
          job.payment_link_id === link.id &&
          Number(job.payment_link_revision) === link.revision &&
          ['accepted', 'delivered', 'ambiguous', 'skipped'].includes(
            job.state as string
          )
      )
    ...
    const milestone = selectDueMilestone({
      anchorDate: sentOn,
      today,
      activatedOn: linkFollowUp.activatedOn,
      milestones: [
        { key: 'link-after-1', offsetDays: 1 },
        { key: 'link-after-3', offsetDays: 3 },
      ],
      handledKeys: handled,
      catchUpDays: 1,
    });
      ...
      .upsert(
        {
          ...
          payment_link_revision: link.revision,
          contact_id: link.invoice.contact_id,
          kind: 'payment_link_follow_up',
          subject_cycle_id: subjectCycleId,
          milestone_key: milestone.key,
          business_key: createReminderBusinessKey({
            kind: 'payment_link_follow_up',
          ...
          effective_due_on: sentOn,
          coordination_on: today,
          activation_generation: linkFollowUp.generation,
        },
        { onConflict: 'account_id,business_key', ignoreDuplicates: true }
      )
      .select('id');
    if (error)
      summary.notes.push(
        `payment link ${link.id}: queue failed — ${error.message}`
      );
    else if (data?.length) summary.queued++;
  }
}

async function processCommitmentOrLinkJob({
  admin,
  job,
  account,
  now,
  ...
  if (settingError || !rawSetting) {
    await finishJob(admin, job, 'blocked', {
      reason: { code: 'commitment_settings_unavailable' },
    });
    summary.blocked++;
    return;
  }
  const prefix =
    job.kind === 'promise_to_pay'
      ? 'promise_to_pay_reminders'
      : 'payment_link_follow_up';
  const settings = commitmentSettings(
    rawSetting as Record<string, unknown>,
    prefix
  );
  const locale = resolveAccountLocale(account);
  ...
  if (
    ...
    await finishJob(admin, job, 'skipped', {
      reason: { code: 'commitment_lifecycle_no_longer_active' },
    });
    summary.skipped++;
    return;
  }
  ...
  const { data: invoice, error: invoiceError } = await admin
    ...
    .select(
      'id, contact_id, invoice_number, balance, state, requires_refund_review, contact:contacts(id, name, phone)'
    )
    .eq('account_id', job.account_id)
  ...
  if (
    ...
    !isCollectibleInvoice({
      ...
      requiresRefundReview: current.requires_refund_review,
    })
  ) {
    await finishJob(admin, job, 'skipped', {
      reason: { code: 'invoice_no_longer_collectible' },
    });
    summary.skipped++;
    return;
  }
  ...
  let template = TEMPLATE_CONTRACTS.payment_promise_upcoming;
  let params: string[];
  ...
  if (job.kind === 'promise_to_pay') {
    const { data: commitment, error } = await admin
      .from('invoice_collection_commitments')
      .select(
        'id, invoice_id, contact_id, kind, state, amount, promised_on, revision, payment_allocation_snapshot'
      )
      .eq('account_id', job.account_id)
      .eq('invoice_id', current.id)
      .eq('id', job.collection_commitment_id)
      .maybeSingle();
    if (error) throw error;
    const item = commitment as unknown as CollectionCommitment | null;
    if (
      !item ||
      item.invoice_id !== current.id ||
      item.contact_id !== current.contact_id ||
      item.kind !== 'promise_to_pay' ||
      item.revision !== job.commitment_revision ||
      !item.promised_on ||
      item.amount === null ||
      !['open', 'broken'].includes(item.state) ||
      (item.state === 'broken') !== (job.milestone_key === 'promise-broken')
    ) {
      await finishJob(admin, job, 'skipped', {
        reason: { code: 'promise_changed_or_resolved' },
      });
      summary.skipped++;
      return;
    }
    const { data: otherOpenCommitments, error: otherOpenCommitmentsError } =
      await admin
        .from('invoice_collection_commitments')
        .select('id')
        .eq('account_id', job.account_id)
    ...
    if ((otherOpenCommitments ?? []).length > 0) {
      await finishJob(admin, job, 'deferred', {
        reason: { code: 'invoice_hold_open' },
        nextAttemptAt: new Date(now.getTime() + 24 * 60 * 60_000).toISOString(),
      });
      summary.deferred++;
      return;
    }
    const result = await admin.rpc('reconcile_invoice_collection_commitment', {
      p_id: item.id,
    });
    if (result.error) throw result.error;
    if (
      (job.milestone_key === 'promise-broken' && result.data !== 'broken') ||
      (job.milestone_key !== 'promise-broken' && result.data !== 'open')
    ) {
      await finishJob(admin, job, 'skipped', {
        reason: { code: 'promise_payment_reconciled' },
      });
      summary.skipped++;
      return;
    }
    const latest =
      job.milestone_key === 'promise-broken'
        ? selectDueMilestone({
            anchorDate: item.promised_on,
            today,
            activatedOn: settings.activatedOn,
            milestones: [{ key: 'promise-broken', offsetDays: 1 }],
            handledKeys: [],
            catchUpDays: 1,
          })
        : selectDueMilestone({
    ...
    if (!latest || latest.key !== job.milestone_key) {
      await finishJob(admin, job, 'skipped', {
        reason: { code: 'promise_milestone_superseded' },
      });
      summary.skipped++;
      return;
    }
    template =
      job.milestone_key === 'promise-broken'
        ? TEMPLATE_CONTRACTS.payment_promise_missed
        : TEMPLATE_CONTRACTS.payment_promise_upcoming;
    const allocation = await admin.rpc(
      'invoice_commitment_payment_allocations',
      { p_invoice_id: item.invoice_id }
    );
    if (allocation.error) throw allocation.error;
    promiseReminderAmount = Math.min(
      Number(current.balance),
      Math.max(
        Number(item.amount) -
          Math.max(
            Number(allocation.data ?? 0) -
              Number(item.payment_allocation_snapshot ?? 0),
            0
          ),
        0
      )
    );
    if (promiseReminderAmount <= 0) {
      await finishJob(admin, job, 'skipped', {
        reason: { code: 'promise_payment_reconciled' },
      });
      summary.skipped++;
    ...
    params = [
      ...
      buildFormatters(locale).money(promiseReminderAmount),
      buildFormatters(locale).date(item.promised_on),
      account.legalBusinessName!,
    ];
  } else {
    ...
    const { data: link, error } = await admin
      .from('razorpay_payment_links')
      .select(
        'id, invoice_id, revision, expected_amount, short_url, expires_at, status, last_sent_at, last_whatsapp_message_id'
      )
      .eq('account_id', job.account_id)
      .eq('invoice_id', current.id)
    ...
    if (
      !item ||
      item.invoice_id !== current.id ||
      item.revision !== job.payment_link_revision ||
      item.status !== 'created' ||
      !item.short_url ||
      !item.last_sent_at ||
      !item.last_whatsapp_message_id ||
      new Date(item.expires_at) <= now ||
      Number(item.expected_amount) !== Number(current.balance)
    ) {
      await finishJob(admin, job, 'skipped', {
        reason: { code: 'payment_link_not_current' },
      });
      summary.skipped++;
      return;
    }
    const sentOn = todayInTz(locale.timeZone, new Date(item.last_sent_at));
    const latest = selectDueMilestone({
      anchorDate: sentOn,
      today,
      activatedOn: settings.activatedOn,
      milestones: [
    ...
    if (!latest || latest.key !== job.milestone_key) {
      await finishJob(admin, job, 'skipped', {
        reason: { code: 'link_milestone_superseded' },
      });
      summary.skipped++;
      return;
    }
    params = [
      current.contact?.name?.trim() || 'there',
      buildFormatters(locale).money(Number(current.balance)),
      current.invoice_number ?? `#${current.id.slice(0, 8).toUpperCase()}`,
      buildFormatters(locale).dateTime(item.expires_at),
      account.legalBusinessName!,
    ];
    messageParams = {
      body: params,
      buttonParams: { 0: paymentLinkButtonParam(item.short_url) },
    };
  }
  ...
  try {
    ...
    const result = await engineSendTemplate({
      beforeSend: async () => {
        ...
        const { data: finalSetting, error: finalSettingError } = await admin
          ...
          .maybeSingle();
        if (finalSettingError)
          throw new ReminderDeferredError('commitment_settings_unavailable');
        if (!finalSetting)
          throw new ReminderNoLongerEligibleError(
            'commitment_lifecycle_no_longer_active'
          );
        const finalSettings = commitmentSettings(
          finalSetting as Record<string, unknown>,
          prefix
        );
        if (
          !finalSettings.enabled ||
          !finalSettings.activatedAt ||
          !finalSettings.activatedOn ||
          finalSettings.generation !== job.activation_generation
        )
          throw new ReminderNoLongerEligibleError(
            'commitment_lifecycle_no_longer_active'
          );
        if (
          ...
          throw new ReminderDeferredError('outside_send_window');
        const { data: finalInvoice, error: finalInvoiceError } = await admin
          .from('invoice_balances')
          .select('balance, state, requires_refund_review')
          .eq('account_id', job.account_id)
          .eq('id', job.invoice_id)
          .maybeSingle();
        if (finalInvoiceError)
          throw new ReminderDeferredError('invoice_balance_unavailable');
        if (
          !finalInvoice ||
          !isCollectibleInvoice({
            state: finalInvoice.state as string,
            balance: Number(finalInvoice.balance),
            requiresRefundReview: Boolean(finalInvoice.requires_refund_review),
          })
        )
          throw new ReminderNoLongerEligibleError(
            'invoice no longer collectible'
          );
        if (await needsAutoPayReconciliation(admin, job.account_id, current.id))
          throw new ReminderDeferredError('autopay_reconciliation_required');
        if (job.kind === 'promise_to_pay') {
          const { data: finalPromise, error: finalPromiseLookupError } =
            await admin
              .from('invoice_collection_commitments')
              .select(
                'id, kind, state, amount, promised_on, revision, payment_allocation_snapshot'
              )
              .eq('account_id', job.account_id)
              .eq('invoice_id', current.id)
              .eq('id', job.collection_commitment_id)
              .maybeSingle();
          if (finalPromiseLookupError)
            throw new ReminderDeferredError('promise_lookup_unavailable');
          if (
            !finalPromise ||
            finalPromise.kind !== 'promise_to_pay' ||
            Number(finalPromise.revision) !== job.commitment_revision ||
            (job.milestone_key === 'promise-broken'
              ? finalPromise.state !== 'broken'
              : finalPromise.state !== 'open')
          )
            throw new ReminderNoLongerEligibleError(
              'promise_changed_or_resolved'
            );
          const { data: finalOpenHolds, error: finalOpenHoldsError } =
            await admin
              .from('invoice_collection_commitments')
              .select('id')
              .eq('account_id', job.account_id)
              .eq('invoice_id', current.id)
              .eq('state', 'open')
              .neq('id', job.collection_commitment_id);
          if (finalOpenHoldsError)
            throw new ReminderDeferredError('commitment_lookup_unavailable');
          if ((finalOpenHolds ?? []).length > 0)
            throw new ReminderDeferredError('invoice_hold_open');
          const reconciliation = await admin.rpc(
            'reconcile_invoice_collection_commitment',
            { p_id: job.collection_commitment_id }
          );
          if (reconciliation.error)
            throw new ReminderDeferredError(
              'promise_reconciliation_unavailable'
            );
          if (
            job.milestone_key === 'promise-broken'
              ? reconciliation.data !== 'broken'
              : reconciliation.data !== 'open'
          )
            throw new ReminderNoLongerEligibleError(
              'promise_payment_reconciled'
            );
          const { data: finalPromiseRow, error: finalPromiseError } =
            await admin
              .from('invoice_collection_commitments')
              .select('amount, payment_allocation_snapshot')
              .eq('account_id', job.account_id)
              .eq('invoice_id', current.id)
              .eq('id', job.collection_commitment_id)
              .maybeSingle();
          const finalAllocation = await admin.rpc(
            'invoice_commitment_payment_allocations',
          ...
          const finalAmount = Math.min(
            ...
            Math.max(
              Number(finalPromiseRow?.amount ?? 0) -
                Math.max(
                  Number(finalAllocation.data ?? 0) -
                    Number(finalPromiseRow?.payment_allocation_snapshot ?? 0),
                  0
                ),
              0
            )
          );
          if (finalPromiseError || finalAllocation.error)
            throw new ReminderDeferredError('promise_balance_unavailable');
          if (finalAmount <= 0)
            throw new ReminderNoLongerEligibleError(
              'promise_payment_reconciled'
            );
          if (finalAmount !== promiseReminderAmount)
            throw new ReminderDeferredError('promise_residual_changed');
        } else {
          const {
            data: finalOpenCommitments,
            error: finalOpenCommitmentsError,
          } = await admin
            .from('invoice_collection_commitments')
            .select('id')
            .eq('account_id', job.account_id)
            .eq('invoice_id', current.id)
            .eq('state', 'open');
          if (finalOpenCommitmentsError)
            throw new ReminderDeferredError('commitment_lookup_unavailable');
          if ((finalOpenCommitments ?? []).length > 0)
            throw new ReminderDeferredError('invoice_commitment_or_hold_open');
          const { data: finalLink, error: finalLinkError } = await admin
            .from('razorpay_payment_links')
            .select(
              'id, invoice_id, revision, expected_amount, short_url, expires_at, status, last_sent_at, last_whatsapp_message_id'
            )
            ...
            .maybeSingle();
          if (finalLinkError)
            throw new ReminderDeferredError('payment_link_lookup_unavailable');
          if (
            !finalLink ||
            Number(finalLink.revision) !== job.payment_link_revision ||
            finalLink.status !== 'created' ||
            !finalLink.short_url ||
            !finalLink.last_sent_at ||
            !finalLink.last_whatsapp_message_id ||
            new Date(finalLink.expires_at) <= now ||
            Number(finalLink.expected_amount) !==
              Number(finalInvoice.balance) ||
            new Date(finalLink.last_sent_at) <
              new Date(finalSettings.activatedAt)
          )
            throw new ReminderNoLongerEligibleError('payment_link_not_current');
          const finalMilestone = selectDueMilestone({
            anchorDate: todayInTz(
              locale.timeZone,
              new Date(finalLink.last_sent_at)
            ),
            today,
          ...
          if (!finalMilestone || finalMilestone.key !== job.milestone_key)
            throw new ReminderNoLongerEligibleError(
              'link_milestone_superseded'
            );
          params[1] = buildFormatters(locale).money(
            Number(finalInvoice.balance)
          );
          params[3] = buildFormatters(locale).dateTime(finalLink.expires_at);
          if (messageParams?.buttonParams) {
            messageParams.buttonParams[0] = paymentLinkButtonParam(
              finalLink.short_url
            );
          }
        }
    ...
    if (error instanceof ReminderNoLongerEligibleError) {
      ...
      summary.skipped++;
    } else if (error instanceof ReminderDeferredError) {
      await finishJob(admin, job, 'deferred', {
        reason: { code: error.message },
        nextAttemptAt: new Date(now.getTime() + 60 * 60_000).toISOString(),
      });
...
export async function runLifecycleReminderWorker(
  ...
  for (const rawSetting of (rawSettings ?? []) as Record<string, unknown>[]) {
    ...
    const membershipPostExpiry = postExpirySettings(
      rawSetting,
      'membership_post_expiry'
    );
    const servicePostExpiry = postExpirySettings(
      rawSetting,
      'service_post_expiry'
    );
    const promiseToPay = commitmentSettings(
      rawSetting,
      'promise_to_pay_reminders'
    );
    const paymentLinkFollowUp = commitmentSettings(
      rawSetting,
      'payment_link_follow_up'
    );
    const retentionEnabled =
      rawSetting.session_pack_reminders_enabled === true ||
      rawSetting.freeze_return_reminders_enabled === true ||
      rawSetting.membership_win_back_enabled === true ||
      rawSetting.service_win_back_enabled === true;
    const transactionEventsEnabled =
      rawSetting.payment_confirmations_enabled === true ||
      rawSetting.autopay_recovery_enabled === true;
    if (
      (!settings.enabled || !settings.activatedOn || !settings.activatedAt) &&
      (!membershipPostExpiry.enabled ||
        !membershipPostExpiry.activatedOn ||
        !membershipPostExpiry.activatedAt) &&
      (!servicePostExpiry.enabled ||
        !servicePostExpiry.activatedOn ||
        !servicePostExpiry.activatedAt) &&
      (!promiseToPay.enabled ||
        !promiseToPay.activatedOn ||
        !promiseToPay.activatedAt) &&
      (!paymentLinkFollowUp.enabled ||
        !paymentLinkFollowUp.activatedOn ||
        !paymentLinkFollowUp.activatedAt) &&
    ...
    const typedAccount = {
      ...(account as Omit<AccountRow, 'legalBusinessName'>),
      legalBusinessName: legalIdentity.ok ? legalIdentity.name : null,
    };
    accountContexts.set(settings.accountId, {
      account: typedAccount,
      ...(!legalIdentity.ok && { legalIdentityError: legalIdentity.code }),
    });
    ...
    const today = todayInTz(locale.timeZone, now);
    try {
      await queuePostExpiryCandidates({
        admin,
        account: typedAccount,
        rawSetting,
        today,
      ...
      summary.infrastructureFailures++;
      summary.notes.push(
        `account ${settings.accountId}: post-expiry source query failed — ${error instanceof Error ? error.message : String(error)}`
      );
    }
    try {
      await queueCommitmentAndLinkCandidates({
        admin,
        account: typedAccount,
        rawSetting,
        today,
      ...
      summary.infrastructureFailures++;
      summary.notes.push(
        `account ${settings.accountId}: commitment source query failed — ${error instanceof Error ? error.message : String(error)}`
      );
    }
    try {
      await queueRetentionCandidates({
        admin,
        account: typedAccount,
        rawSetting,
        today,
      ...
      summary.infrastructureFailures++;
      summary.notes.push(
        `account ${settings.accountId}: retention source query failed — ${error instanceof Error ? error.message : String(error)}`
      );
    }
    if (!settings.enabled || !settings.activatedOn || !settings.activatedAt)
      continue;
    if (
      !isWithinReminderSendWindow(
        hourInTz(locale.timeZone, now),
    ...
    if (invoicesError || installmentsError) {
      summary.infrastructureFailures++;
      summary.notes.push(
        `account ${settings.accountId}: collection source query failed — ${(invoicesError ?? installmentsError)?.message}`
      );
      continue;
    }
    ...
    for (const invoice of (invoices ?? []) as unknown as InvoiceCandidate[]) {
      if (
        ...
        continue;
      const effectiveDueOn = todayInTz(
        locale.timeZone,
      ...
      if (queueError) {
        summary.notes.push(
          `invoice ${invoice.id}: queue failed — ${queueError.message}`
        );
    ...
    for (const installment of (installments ??
      ...
      if (
        ...
        !collectibleInvoices.has(installment.invoice_id)
      )
        continue;
      const milestone = selectDueMilestone({
      ...
      if (queueError) {
        summary.notes.push(
          `installment ${installment.id}: queue failed — ${queueError.message}`
        );
      } else if (created?.length) {
        summary.queued++;
      }
  ...
  for (const job of (claimed ?? []) as LifecycleReminderJob[]) {
    try {
      ...
      if (
        job.kind === 'membership_post_expiry' ||
        job.kind === 'service_post_expiry'
      ) {
      ...
      if (
        job.kind === 'promise_to_pay' ||
        job.kind === 'payment_link_follow_up'
      ) {
        await processCommitmentOrLinkJob({ admin, job, account, now, summary });
        continue;
      }
      if (
        job.kind === 'payment_confirmation' ||
        job.kind === 'autopay_recovery'
      ) {
        const outcome = await processTransactionEventJob({
          ...
          reserveDailyClaim: async (sendOn) => {
            ...
            if (error || (data !== 'reserved' && data !== 'deferred'))
              throw new Error(
                error?.message ?? 'daily coordination lease lost'
              );
            return data;
          },
        });
      ...
      if (
        ...
        job.kind === 'freeze_return' ||
        job.kind === 'membership_win_back' ||
        job.kind === 'service_win_back'
      ) {
      ...
      const { data: currentSettingsRow, error: currentSettingsError } =
        await admin
          .from('renewal_reminder_settings')
          .select(SETTINGS_SELECT)
          .eq('account_id', job.account_id)
      ...
      const currentSettings = asSettings(
        currentSettingsRow as Record<string, unknown>
      );
      if (
        !currentSettings.enabled ||
        String(
          (currentSettingsRow as Record<string, unknown>)
            .invoice_collection_generation
      ...
      if (
        !isWithinReminderSendWindow(
          hourInTz(locale.timeZone, now),
          currentSettings.sendWindowStart,
          currentSettings.sendWindowEnd
        )
      ) {
        await finishJob(admin, job, 'deferred', {
          reason: { code: 'outside_send_window' },
      ...
      const { data: invoice, error: invoiceError } = await admin
        .from('invoice_balances')
        .select(
          'id, contact_id, membership_id, invoice_number, balance, state, requires_refund_review, contact:contacts(id, name, phone)'
        )
        .eq('account_id', job.account_id)
        .eq('id', job.invoice_id)
        .maybeSingle();
      ...
      const { data: activeCommitments, error: activeCommitmentsError } =
        await admin
          .from('invoice_collection_commitments')
          .select('id')
          .eq('account_id', job.account_id)
      ...
      if (activeCommitmentsError) {
        await finishJob(admin, job, 'blocked', {
          reason: { code: 'commitment_hold_lookup_unavailable' },
        });
        summary.infrastructureFailures++;
        summary.blocked++;
        continue;
      ...
      try {
        autopayHold = await needsAutoPayReconciliation(
          admin,
          job.account_id,
          current.id
        );
      ...
      const { data: lifecycleHistory, error: lifecycleHistoryError } =
        await admin
          .from('lifecycle_reminder_jobs')
          .select('milestone_key, state')
          .eq('account_id', job.account_id)
      ...
      if (job.kind === 'installment_overdue') {
        ...
        if (
          installmentError ||
          !installment ||
          installment.invoice_id !== job.invoice_id
        ) {
          await finishJob(admin, job, 'skipped', {
            reason: { code: 'installment_no_longer_current' },
          });
          summary.skipped++;
          continue;
        }
        latestMilestone = selectDueMilestone({
          ...
          milestones: currentSettings.overdueDays.map((days) => ({
            key: `overdue-${days}`,
            offsetDays: days,
          })),
        ...
        if (installmentForInvoice) {
          await finishJob(admin, job, 'skipped', {
            reason: { code: 'fixed_installment_owns_invoice' },
          });
          summary.skipped++;
          continue;
        }
        latestMilestone = selectDueMilestone({
          ...
          activatedOn: currentSettings.activatedOn!,
          milestones: invoiceMilestones(
            currentSettings.beforeDueDays,
            currentSettings.overdueDays
          ),
          handledKeys: (lifecycleHistory ?? [])
            .filter((row) =>
              ['accepted', 'delivered', 'ambiguous', 'skipped'].includes(
      ...
      const { data: dailyReservation, error: dailyReservationError } =
        await admin.rpc('reserve_lifecycle_reminder_daily_claim', {
          p_job_id: job.id,
          p_worker_id: job.lease_owner,
          p_lease_generation: job.lease_generation,
          p_send_on: today,
      ...
      try {
        ...
        const { whatsapp_message_id } = await engineSendTemplate({
          beforeSend: async () => {
            ...
            const { data: finalSettingsRow, error: finalSettingsError } =
              await admin
                .from('renewal_reminder_settings')
                .select(
                  'account_id, invoice_collection_enabled, invoice_collection_activated_on, invoice_collection_activated_at, invoice_collection_generation, invoice_collection_before_due_days, invoice_collection_overdue_days, invoice_collection_catch_up_days, invoice_collection_send_window_start, invoice_collection_send_window_end'
            ...
            if (finalSettingsError || !finalSettingsRow) {
              throw new ReminderSetupChangedError(
                'invoice collection settings unavailable'
              );
            }
            const finalSettings = asSettings(
              finalSettingsRow as Record<string, unknown>
            );
            if (
              !finalSettings.enabled ||
              String(
              ...
              throw new ReminderNoLongerEligibleError(
                'invoice collection disabled or regenerated'
              );
            }
            if (
              !isWithinReminderSendWindow(
                hourInTz(locale.timeZone, new Date()),
                finalSettings.sendWindowStart,
                finalSettings.sendWindowEnd
              )
            ) {
              throw new ReminderSetupChangedError('send window changed');
            }
            const { data: latestInvoice, error: latestInvoiceError } =
              await admin
                .from('invoice_balances')
                .select(
                  'id, invoice_number, balance, state, requires_refund_review'
                )
                .eq('account_id', job.account_id)
                .eq('id', job.invoice_id)
                .maybeSingle();
            ...
            if (
              ...
              !isCollectibleInvoice({
                ...
                requiresRefundReview: Boolean(
                  latestInvoice.requires_refund_review
                ),
              })
            ) {
              throw new ReminderNoLongerEligibleError(
            ...
            if (
              await needsAutoPayReconciliation(
                admin,
                job.account_id,
              ...
              throw new ReminderNoLongerEligibleError(
                'autopay reconciliation required'
              );
            }
            const { data: finalCommitments, error: finalCommitmentsError } =
              await admin
                .from('invoice_collection_commitments')
                .select('id')
                .eq('account_id', job.account_id)
            ...
            const finalToday = todayInTz(locale.timeZone, new Date());
            const finalLatest =
              job.kind === 'installment_overdue'
                ? selectDueMilestone({
                    anchorDate: job.effective_due_on,
                    today: finalToday,
                    activatedOn: finalSettings.activatedOn!,
                    milestones: finalSettings.overdueDays.map((days) => ({
                      key: `overdue-${Math.abs(days)}`,
                      offsetDays: Math.abs(days),
                    })),
                ...
                : selectDueMilestone({
                    ...
                    activatedOn: finalSettings.activatedOn!,
                    milestones: invoiceMilestones(
                      finalSettings.beforeDueDays,
                      finalSettings.overdueDays
                    ),
                    handledKeys: finalHandled,
                    catchUpDays: finalSettings.catchUpDays,
                  });
            ...
            if (job.installment_plan_id) {
              ...
              if (
                !installment ||
                installment.invoice_id !== job.invoice_id ||
                installment.second_due_on !== job.effective_due_on
              ) {
                throw new ReminderNoLongerEligibleError(
                  'installment promise changed'
                );
              }
            }
            templateParams[1] =
              (latestInvoice.invoice_number as string | null) ??
              templateParams[1];
            ...
            if (markError || marked !== true) {
              throw new Error(
                markError?.message ?? 'provider attempt lease lost'
              );
            }
            providerRequestStarted = true;
          },
        ...
        try {
          ...
          summary.ambiguous++;
          summary.notes.push(
            `job ${job.id}: provider accepted but durable completion failed — ${error instanceof Error ? error.message : String(error)}`
          );
        }
      } catch (error) {
        try {
