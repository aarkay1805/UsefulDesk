...
type Admin = ReturnType<
  typeof import('@/lib/automations/admin-client').supabaseAdmin
>;
...
export type TransactionEventResult =
  'accepted' | 'blocked' | 'skipped' | 'deferred' | 'ambiguous' | 'failed';
...
function eventSettings(row: Record<string, unknown> | null): Settings {
  return {
    ...
    payment_confirmations_activated_at:
      typeof row?.payment_confirmations_activated_at === 'string'
        ? row.payment_confirmations_activated_at
        : null,
    payment_confirmations_generation:
      typeof row?.payment_confirmations_generation === 'string'
        ? row.payment_confirmations_generation
        : null,
    autopay_recovery_enabled: row?.autopay_recovery_enabled === true,
    autopay_recovery_activated_at:
      typeof row?.autopay_recovery_activated_at === 'string'
        ? row.autopay_recovery_activated_at
        : null,
    autopay_recovery_generation:
      typeof row?.autopay_recovery_generation === 'string'
        ? row.autopay_recovery_generation
        : null,
  };
}
...
function retryAt(attemptCount: number, now: Date) {
  return new Date(
    now.getTime() + Math.min(60, 2 ** Math.min(attemptCount, 5)) * 60_000
  ).toISOString();
}
...
async function setup(
  ...
  const [{ data: config }, { data: templates }] = await Promise.all([
    ...
    admin
      ...
      .select('*')
      .eq('account_id', job.account_id)
      .eq('name', TEMPLATE_CONTRACTS[contract].payload.name),
  ]);
  const readiness = evaluateTemplateReadiness(templates, contract, 'en_US');
  if (!config || config.status !== 'connected')
    return { code: 'whatsapp_not_connected' as const };
...
export async function processTransactionEventJob(input: {
  ...
  const { data: rawSettings, error: settingsError } = await admin
    ...
    .select(
      'payment_confirmations_enabled, payment_confirmations_activated_at, payment_confirmations_generation, autopay_recovery_enabled, autopay_recovery_activated_at, autopay_recovery_generation'
    )
    .eq('account_id', job.account_id)
    .maybeSingle();
  if (settingsError || !rawSettings) {
    await finish('blocked', {
      reason: { code: 'transaction_event_settings_unavailable' },
    });
    return 'blocked';
  }
  ...
  if (
    isConfirmation &&
    (!settings.payment_confirmations_enabled ||
      settings.payment_confirmations_generation !== job.activation_generation)
  ) {
    await finish('skipped', {
      reason: { code: 'payment_confirmations_disabled_or_regenerated' },
    });
    return 'skipped';
  }
  if (
    !isConfirmation &&
    (!settings.autopay_recovery_enabled ||
      settings.autopay_recovery_generation !== job.activation_generation)
  ) {
    await finish('skipped', {
      reason: { code: 'autopay_recovery_disabled_or_regenerated' },
    });
    return 'skipped';
  }
  ...
  if (isConfirmation) {
    const { data: payment, error } = await admin
      .from('payments')
      .select(
        'id, account_id, contact_id, invoice_id, amount, status, created_at, paid_at, payment_purpose, invoice:invoices(invoice_number), contact:contacts(name)'
      )
      .eq('account_id', job.account_id)
    ...
    if (
      !row ||
      row.id !== job.payment_id ||
      row.contact_id !== job.contact_id ||
      row.status !== 'paid' ||
      !settings.payment_confirmations_activated_at ||
      new Date(row.created_at) <
        new Date(settings.payment_confirmations_activated_at)
    ) {
      await finish('skipped', {
        reason: { code: 'payment_not_a_new_committed_confirmation' },
      });
      return 'skipped';
    }
    contactName = row.contact?.name?.trim() || contactName;
    const reason = (
      job as LifecycleReminderJob & {
        reason?: { renewed?: boolean; period_end?: string | null };
      }
    ).reason;
    const renewed =
      reason?.renewed === true && typeof reason.period_end === 'string';
    const reference =
      row.invoice?.invoice_number ??
      `payment ${row.id.slice(0, 8).toUpperCase()}`;
    if (renewed && reason?.period_end) {
      params = [
        contactName,
        fmt.money(Number(row.amount)),
        reference,
        fmt.date(reason.period_end),
        account.legalBusinessName!,
      ];
      contract = 'payment_membership_renewal_confirmation';
    } else {
      params = [
        contactName,
        fmt.money(Number(row.amount)),
    ...
    const { data: failure, error } = await admin
      ...
      .select(
        'id, event_kind, invoice_id, contact_id, observed_at, superseded_at, mandate:payment_mandates(id, membership_id, status, provider_subscription_status)'
      )
      .eq('account_id', job.account_id)
      .eq('id', job.autopay_failure_event_id)
      .maybeSingle();
    if (error) throw error;
    const event = failure as {
      id: string;
      event_kind: 'retry_pending' | 'terminal';
      invoice_id: string | null;
      contact_id: string;
    ...
    if (
      !event ||
      event.id !== job.autopay_failure_event_id ||
      event.contact_id !== job.contact_id ||
      event.superseded_at ||
      !event.mandate
    ) {
      await finish('skipped', {
        reason: { code: 'autopay_failure_superseded_or_missing' },
      });
      return 'skipped';
    }
    const { data: invoiceData, error: invoiceError } = event.invoice_id
      ? await admin
          ...
          .select(
            'id, invoice_number, balance, state, requires_refund_review, contact:contacts(name, phone)'
          )
          .eq('account_id', job.account_id)
    ...
    if (event.event_kind === 'retry_pending') {
      if (
        (event.mandate.status !== 'active' &&
          event.mandate.status !== 'pending') ||
        event.mandate.provider_subscription_status !== 'pending'
      ) {
        await finish('skipped', {
          reason: { code: 'autopay_retry_state_changed' },
        });
        return 'skipped';
      }
      params = [contactName, 'your membership', account.legalBusinessName!];
      ...
      const { data: openCommitments, error: commitmentsError } = await admin
      ...
      const { data: healthy, error: healthyError } = await admin
        .from('payment_mandates')
        .select('id')
        .eq('account_id', job.account_id)
        .eq('membership_id', event.mandate.membership_id)
        .eq('status', 'active')
        .limit(1);
      if (healthyError) throw healthyError;
      if ((healthy ?? []).length) {
        await finish('skipped', {
          reason: { code: 'healthy_mandate_now_covers_obligation' },
        });
        return 'skipped';
      }
      params = [
        ...
        fmt.money(Number(invoice.balance)),
        account.legalBusinessName!,
      ];
  ...
  if ('code' in readiness) {
    await finish('blocked', {
      reason: { code: readiness.code ?? 'template_not_ready' },
    });
    return 'blocked';
  }
  ...
  try {
    ...
    const result = await sendReminderTemplate({
      beforeSend: async () => {
        ...
        const { data: finalSettings, error: finalSettingsError } = await admin
          .from('renewal_reminder_settings')
          .select(
            'payment_confirmations_enabled, payment_confirmations_generation, autopay_recovery_enabled, autopay_recovery_generation'
          )
          .eq('account_id', job.account_id)
          .maybeSingle();
        if (finalSettingsError)
          throw new Error('transaction_event_settings_lookup_unavailable');
        const final = eventSettings(
          finalSettings as Record<string, unknown> | null
        );
        if (
          isConfirmation
            ? !final.payment_confirmations_enabled ||
              final.payment_confirmations_generation !==
                job.activation_generation
            : !final.autopay_recovery_enabled ||
              final.autopay_recovery_generation !== job.activation_generation
        )
        ...
        if (isConfirmation) {
          ...
          if (latestPaymentError)
            throw new Error('payment_confirmation_lookup_unavailable');
          const currentPayment = latestPayment as {
            status: string;
            invoice_id: string | null;
          } | null;
          ...
          if (currentPayment.invoice_id) {
            ...
            if (
              (latestInvoice as { requires_refund_review: boolean })
                .requires_refund_review
            )
              throw new Error('payment_confirmation_state_changed');
          }
        } else {
          const { data: latest, error: latestFailureError } = await admin
            .from('razorpay_autopay_failure_events')
            .select(
              'superseded_at, event_kind, invoice_id, mandate:payment_mandates(membership_id, status, provider_subscription_status)'
            )
            .eq('account_id', job.account_id)
            .eq('id', job.autopay_failure_event_id)
            .maybeSingle();
          if (latestFailureError)
            throw new Error('autopay_failure_lookup_unavailable');
          const currentFailure = latest as {
            superseded_at: string | null;
            event_kind: 'retry_pending' | 'terminal';
            invoice_id: string | null;
            mandate: {
          ...
          if (
            !currentFailure ||
            currentFailure.superseded_at ||
            !currentFailure.mandate
          )
            throw new Error('autopay_failure_superseded');
          if (currentFailure.event_kind === 'retry_pending') {
            if (
              (currentFailure.mandate.status !== 'active' &&
                currentFailure.mandate.status !== 'pending') ||
              currentFailure.mandate.provider_subscription_status !== 'pending'
            ) {
              throw new Error('autopay_retry_state_changed');
            }
          } else {
            if (!currentFailure.invoice_id)
              throw new Error('autopay_terminal_state_changed');
            const [
              { data: latestInvoice, error: latestInvoiceError },
              { data: openCommitments, error: commitmentsError },
              { data: healthy, error: healthyError },
            ] = await Promise.all([
              admin
                .from('invoice_balances')
                .select(
                ...
                .eq('account_id', job.account_id)
                .eq('id', currentFailure.invoice_id)
                .maybeSingle(),
              admin
                .from('invoice_collection_commitments')
                .select('id')
                .eq('account_id', job.account_id)
                .eq('invoice_id', currentFailure.invoice_id)
                .eq('state', 'open'),
              admin
                .from('payment_mandates')
                .select('id')
                .eq('account_id', job.account_id)
                .eq('membership_id', currentFailure.mandate.membership_id)
                .eq('status', 'active')
                .limit(1),
            ]);
            if (latestInvoiceError || healthyError)
              throw new Error('autopay_terminal_state_lookup_unavailable');
            if (commitmentsError)
              throw new Error('autopay_terminal_commitment_lookup_unavailable');
            if ((openCommitments ?? []).length > 0)
              throw new Error('autopay_terminal_commitment_open');
      ...
      templateName: TEMPLATE_CONTRACTS[contract].payload.name,
    ...
    if (
      ...
      code === 'payment_confirmation_state_changed' ||
      code === 'autopay_failure_superseded' ||
      code === 'autopay_retry_state_changed' ||
      code === 'autopay_terminal_state_changed'
    ) {
