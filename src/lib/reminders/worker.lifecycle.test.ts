...
vi.mock('@/lib/locale/format', () => ({
  ...
  hourInTz: () => 10,
  todayInTz: (_timeZone: string, date?: Date) =>
    h.paymentLinkKind && date?.toISOString().startsWith('2026-09-10')
      ? '2026-09-10'
      : '2026-09-11',
}));
...
class Query {
  ...
  eq(field: string, value: unknown) {
    h.filters.push([this.table, field, value]);
    return this;
  }
  neq(field: string, value: unknown) {
    void value;
    if (this.table === 'invoice_collection_commitments' && field === 'id')
      this.excludesCommitment = true;
    return this;
  }
  ...
  then(resolve: (value: { data: unknown; error: null }) => unknown) {
    const data =
      this.table === 'renewal_reminder_settings'
          ...
          ? [
              {
                ...
                invoice_collection_before_due_days: [0],
                invoice_collection_overdue_days: [1],
                invoice_collection_catch_up_days: 2,
                ...
                membership_post_expiry_activated_on:
                  h.postExpiryKind === 'membership' ? '2026-09-01' : null,
                membership_post_expiry_activated_at:
                  h.postExpiryKind === 'membership'
                    ? '2026-09-01T08:00:00.000Z'
                    : null,
                membership_post_expiry_generation: 'post-generation-1',
                membership_post_expiry_catch_up_days: 2,
                service_post_expiry_enabled: h.postExpiryKind === 'service',
                service_post_expiry_activated_on:
                  h.postExpiryKind === 'service' ? '2026-09-01' : null,
                service_post_expiry_activated_at:
                  h.postExpiryKind === 'service'
                    ? '2026-09-01T08:00:00.000Z'
                    : null,
                service_post_expiry_generation: 'post-generation-1',
                service_post_expiry_catch_up_days: 2,
                promise_to_pay_reminders_enabled:
                  h.commitmentKind === 'promise',
                promise_to_pay_reminders_activated_on:
                  h.commitmentKind === 'promise' ? '2026-09-01' : null,
                promise_to_pay_reminders_activated_at:
                  h.commitmentKind === 'promise'
                    ? '2026-09-01T08:00:00.000Z'
                    : null,
                promise_to_pay_reminders_generation: 'promise-generation-1',
                payment_link_follow_up_enabled: h.paymentLinkKind,
                payment_link_follow_up_activated_on: h.paymentLinkKind
                  ? '2026-09-01'
                  : null,
                payment_link_follow_up_activated_at: h.paymentLinkKind
                  ? '2026-09-01T08:00:00.000Z'
                  : null,
                payment_link_follow_up_generation: 'link-generation-1',
              },
            ]
          : []
        : this.table === 'accounts'
          ? [
              {
                ...
                legal_entity: h.legalIdentityMissing
                  ? null
                  : {
          ...
          : this.table === 'invoice_balances'
            ...
            : this.table === 'memberships'
              ...
              : this.table === 'service_renewal_queue'
                ? [
                    {
                      id: 'service-1',
                      account_id: 'account-1',
                ...
                : this.table === 'invoice_collection_commitments' &&
                    h.commitmentKind === 'promise'
                  ? this.excludesCommitment
                    ? []
                    : [
                        {
                          id: 'promise-1',
                          ...
                          contact_id: 'contact-1',
                          kind: 'promise_to_pay',
                          state: h.promiseBroken ? 'broken' : 'open',
                          amount: 40,
                          promised_on: h.promiseBroken
                            ? '2026-09-10'
                            : '2026-09-11',
                          revision: 1,
                          payment_allocation_snapshot: 0,
                          created_at: '2026-09-11T08:00:00.000Z',
                        },
                      ]
                  : this.table === 'razorpay_payment_links' && h.paymentLinkKind
                    ? [
                        {
                          id: 'link-1',
                          invoice_id: 'invoice-1',
                          revision: 1,
                          expected_amount: 40,
                          short_url: 'https://rzp.io/i/link-1',
                          expires_at: '2026-09-20T00:00:00.000Z',
                          status: 'created',
                          last_sent_at: '2026-09-10T10:00:00.000Z',
                          last_whatsapp_message_id: 'wamid-original',
                          invoice: { contact_id: 'contact-1' },
                        },
                      ]
                    : this.table === 'membership_installment_plans' ||
                        this.table === 'invoice_line_balances' ||
                        this.table === 'payment_mandates' ||
                        this.table === 'membership_periods' ||
                        this.table === 'messages'
                      ? []
                      : this.table === 'conversations'
                        ? [{ id: 'conversation-1' }]
                        : this.table === 'whatsapp_config'
                          ? [{ status: 'connected' }]
                          : this.table === 'message_templates'
                            ? [{}]
                            : this.table === 'lifecycle_reminder_jobs'
                              ? this.fields === 'id'
                                ? [{ id: 'queued-1' }]
                                : []
                              : [];
    const value =
      this.singleMode && Array.isArray(data) ? (data[0] ?? null) : data;
...
describe('runLifecycleReminderWorker', () => {
  it('blocks a claimed job with a structured reason when legal identity is missing', async () => {
  ...
  it('uses the shared durable path for a post-expiry membership, rechecks it at beforeSend, and escalates the final unanswered milestone', async () => {
          ...
          data: [
            {
              ...
              business_key: 'key',
              subject_cycle_id: 'cycle',
              milestone_key: 'expired-7',
              effective_due_on: '2026-09-04',
              activation_generation: 'post-generation-1',
              state: 'leased',
              attempt_count: 0,
              lease_owner: 'worker-1',
              lease_generation: 1,
    ...
    expect(h.engineSend).toHaveBeenCalledWith(
      expect.objectContaining({ templateName: 'gym_membership_post_expiry' })
    );
    expect(h.rpc.mock.calls.map(([name]) => name)).toEqual(
      expect.arrayContaining([
        'reserve_lifecycle_reminder_daily_claim',
  ...
  it('uses the same worker path and exact service contract for a renewable service', async () => {
    ...
    h.rpc.mockImplementation((name: string) => {
      if (name === 'claim_lifecycle_reminder_jobs')
        return Promise.resolve({
          data: [
            {
              id: 'post-service-job-1',
              account_id: 'account-1',
              contact_id: 'contact-1',
              invoice_id: null,
              installment_plan_id: null,
              membership_id: null,
              member_service_id: 'service-1',
    ...
    expect(h.engineSend).toHaveBeenCalledWith(
      expect.objectContaining({ templateName: 'gym_service_post_expiry' })
    );
    expect(summary.accepted).toBe(1);
    h.postExpiryKind = null;
  });
  ...
  it('skips a renewed subject before provider work and records an ambiguous post-expiry request after its boundary', async () => {
    ...
    const { runLifecycleReminderWorker } = await import('./worker');
    const skipped = await runLifecycleReminderWorker(
      new Date('2026-09-11T10:00:00.000Z')
    );
    ...
    const ambiguous = await runLifecycleReminderWorker(
      new Date('2026-09-11T10:00:00.000Z')
    );
  ...
  it('uses the shared worker path for a revisioned promise and marks the provider boundary before sending', async () => {
    h.commitmentKind = 'promise';
    h.promiseBroken = false;
    h.partialAtBoundary = false;
    h.allocationCalls = 0;
    h.rpc.mockReset();
    h.engineSend.mockReset();
    h.rpc.mockImplementation((name: string) => {
          ...
          data: [
            {
              ...
              invoice_id: 'invoice-1',
              installment_plan_id: null,
              collection_commitment_id: 'promise-1',
              commitment_revision: 1,
              kind: 'promise_to_pay',
              business_key: 'key',
              subject_cycle_id: 'cycle',
              milestone_key: 'promise-due',
              effective_due_on: '2026-09-11',
              activation_generation: 'promise-generation-1',
              state: 'leased',
              attempt_count: 0,
              lease_owner: 'worker-1',
              lease_generation: 1,
      ...
      if (name === 'reserve_lifecycle_reminder_daily_claim')
        return Promise.resolve({ data: 'reserved', error: null });
      if (name === 'reconcile_invoice_collection_commitment')
        return Promise.resolve({
          data: h.promiseBroken ? 'broken' : 'open',
          error: null,
        });
      if (name === 'invoice_commitment_payment_allocations')
        return Promise.resolve({
          data: h.partialAtBoundary && ++h.allocationCalls > 1 ? 20 : 0,
          error: null,
        });
      return Promise.resolve({ data: true, error: null });
    });
    h.engineSend.mockImplementation(
      async (args: { beforeSend: () => Promise<void> }) => {
        await args.beforeSend();
        return { whatsapp_message_id: 'wamid-promise-1' };
      }
    );
    const { runLifecycleReminderWorker } = await import('./worker');
    const summary = await runLifecycleReminderWorker(
    ...
    expect(summary.accepted).toBe(1);
    expect(h.engineSend).toHaveBeenCalledWith(
      expect.objectContaining({ templateName: 'gym_payment_promise_upcoming' })
    );
    expect(h.rpc.mock.calls.map(([name]) => name)).toContain(
      'mark_lifecycle_reminder_provider_attempt'
    );
  ...
  it('sends the bounded day-after broken-promise job through the provider boundary', async () => {
    ...
    h.rpc.mockImplementation((name: string) => {
      ...
      if (name === 'reserve_lifecycle_reminder_daily_claim')
        return Promise.resolve({ data: 'reserved', error: null });
      if (name === 'reconcile_invoice_collection_commitment')
        return Promise.resolve({ data: 'broken', error: null });
      if (name === 'invoice_commitment_payment_allocations')
    ...
    h.engineSend.mockImplementation(
      async (args: { beforeSend: () => Promise<void> }) => {
        await args.beforeSend();
        return { whatsapp_message_id: 'wamid-broken-promise' };
      }
    );
    const { runLifecycleReminderWorker } = await import('./worker');
    const summary = await runLifecycleReminderWorker(
    ...
    expect(summary.accepted).toBe(1);
    expect(h.engineSend).toHaveBeenCalledWith(
      expect.objectContaining({ templateName: 'gym_payment_promise_missed' })
    );
    expect(h.rpc.mock.calls.map(([name]) => name)).toContain(
      'mark_lifecycle_reminder_provider_attempt'
    );
  ...
  it('defers, rather than loses, a promise whose payment allocation changes at the provider boundary', async () => {
    ...
    h.engineSend.mockImplementation(
      async (args: { beforeSend: () => Promise<void> }) => {
        await args.beforeSend();
        return { whatsapp_message_id: 'should-not-send' };
      }
    );
    const { runLifecycleReminderWorker } = await import('./worker');
    const summary = await runLifecycleReminderWorker(
    ...
    expect(h.rpc.mock.calls.map(([name]) => name)).not.toContain(
      'mark_lifecycle_reminder_provider_attempt'
    );
    h.commitmentKind = null;
    h.partialAtBoundary = false;
  ...
  it('uses the shared worker path for an accepted, exact-balance payment link', async () => {
          ...
          data: [
            {
              ...
              payment_link_revision: 1,
              kind: 'payment_link_follow_up',
              business_key: 'key',
              subject_cycle_id: 'cycle',
              milestone_key: 'link-after-1',
              effective_due_on: '2026-09-10',
              activation_generation: 'link-generation-1',
              state: 'leased',
              attempt_count: 0,
              lease_owner: 'worker-1',
              lease_generation: 1,
    ...
    h.engineSend.mockImplementation(
      async (args: { beforeSend: () => Promise<void> }) => {
        await args.beforeSend();
        return { whatsapp_message_id: 'wamid-link-1' };
      }
    );
    const { runLifecycleReminderWorker } = await import('./worker');
    const summary = await runLifecycleReminderWorker(
