...
describe('capability guards', () => {
  function accountClient(role: 'viewer' | 'agent' | 'admin') {
    return makeClient({
      user: { id: 'user-1' },
      byTable: {
        profiles: {
  ...
  it('denies viewers operational access', async () => {
    createClient.mockReturnValue(accountClient('viewer'));

    await expect(requireOperationalAccess()).rejects.toMatchObject({
      status: 403,
      message: 'This action requires operational access',
    });
  });

  it('allows agents operational access', async () => {
    createClient.mockReturnValue(accountClient('agent'));

    await expect(requireOperationalAccess()).resolves.toMatchObject({
      userId: 'user-1',
      accountId: 'acct-1',
      role: 'agent',
    });
  ...
  it('denies a removed former member whose profile has no account', async () => {
    const { client } = makeClient({
      user: { id: 'former-user' },
      byTable: {
  ...
  it('requires admin settings capability', async () => {
    createClient.mockReturnValue(accountClient('agent'));
    await expect(requireSettingsAccess()).rejects.toMatchObject({
      status: 403,
      message: 'This action requires settings access',
    });
