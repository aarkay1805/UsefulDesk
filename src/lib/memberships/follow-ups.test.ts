...
describe('defaultReason', () => {
  ...
  it('is trial for a trial row regardless of dates', () => {
    expect(
      defaultReason(member({ is_trial: true, end_date: '2026-07-01' }), today)
    ).toBe('trial');
  });

  it('is renewal when expired or expiring within 7 days', () => {
    expect(defaultReason(member({ end_date: '2026-07-01' }), today)).toBe(
      'renewal'
    ); // expired
    expect(defaultReason(member({ end_date: '2026-07-05' }), today)).toBe(
      'renewal'
    ); // today
    expect(defaultReason(member({ end_date: '2026-07-12' }), today)).toBe(
      'renewal'
    ); // 7d
  });
  ...
  it('is payment when the fee is due and expiry is far off', () => {
    expect(
      defaultReason(
        member({ fee_status: 'due', end_date: '2026-08-01' }),
        today
      )
    ).toBe('payment');
  });
