...
function inv(over: Partial<MembershipPeriodInvoice>): InvoiceFacts {
  ...
  return {
    ...base,
    amount_paid:
      over.amount_paid ?? Number(base.fee_amount) - Number(base.balance),
  };
}

describe('periodStatus', () => {
  it('void state wins over everything', () => {
    expect(periodStatus(inv({ state: 'void', balance: 500 }), TODAY)).toBe(
      'void'
    );
  });
  ...
  it('a covered balance (<= 0) is paid, even for a past cycle', () => {
    expect(
      periodStatus(inv({ balance: 0, period_start: '2026-01-01' }), TODAY)
    ).toBe('paid');
  });
  ...
  it('a cycle that billed and collected nothing is no_charge, not unpaid', () => {
    ...
    expect(
      periodStatus(
        inv({ fee_amount: 0.32, amount_paid: 0, balance: 0.32 }),
        TODAY
      )
    ).toBe('no_charge');
    expect(
      periodStatus(inv({ fee_amount: 0, amount_paid: 0, balance: 0 }), TODAY)
    ).toBe('no_charge');
  });
  ...
  it('a zero-fee stub that DID collect money is paid, not no_charge', () => {
    expect(
      periodStatus(
        inv({ fee_amount: 0.32, amount_paid: 500, balance: 0 }),
        TODAY
      )
    ).toBe('paid');
  });
  ...
  it('a future cycle with an outstanding balance is upcoming', () => {
    expect(
      periodStatus(inv({ balance: 999, period_start: '2026-08-11' }), TODAY)
    ).toBe('upcoming');
  });

  it('a started cycle with a balance is unpaid', () => {
    expect(
      periodStatus(inv({ balance: 999, period_start: '2026-07-01' }), TODAY)
    ).toBe('unpaid');
  });

  it("today's start counts as started (not upcoming)", () => {
    expect(
      periodStatus(inv({ balance: 999, period_start: TODAY }), TODAY)
    ).toBe('unpaid');
  });
  ...
  it("a future cycle that's already paid reads paid, not upcoming", () => {
    ...
    expect(
      periodStatus(inv({ balance: 0, period_start: '2026-09-01' }), TODAY)
    ).toBe('paid');
  });
});
...
describe('invoicePaymentState', () => {
  it('splits paid / due / no_charge at display precision', () => {
    ...
    expect(
      invoicePaymentState(
        inv({ fee_amount: 0.4, amount_paid: 0, balance: 0.4 })
      )
    ).toBe('no_charge');
  });
...
describe('isCollectiblePeriod', () => {
  it('only permits an open period with a positive balance', () => {
    ...
    expect(
      isCollectiblePeriod(inv({ state: 'void', balance: 500 }), 'active')
    ).toBe(false);
  });
...
describe('projectNextInvoice', () => {
  ...
  it('uses the option price (no setup fee), not the current fee', () => {
    ...
    const next = projectNextInvoice(
      { ...baseMembership, fee_amount: 4499 },
      TODAY
    )!;
    expect(next.fee_amount).toBe(3999);
  });
  ...
  it('returns null for trials, cancelled, and option-less memberships', () => {
    expect(
      projectNextInvoice({ ...baseMembership, is_trial: true }, TODAY)
    ).toBeNull();
    expect(
      projectNextInvoice({ ...baseMembership, status: 'cancelled' }, TODAY)
    ).toBeNull();
    expect(
      projectNextInvoice({ ...baseMembership, pricing_option: null }, TODAY)
    ).toBeNull();
  });
  ...
  it('returns null for non-recurring and session-pack plans', () => {
    expect(
      projectNextInvoice(
        {
          ...baseMembership,
          plan: { ...baseMembership.plan!, plan_type: 'non_recurring' },
        },
        TODAY
      )
    ).toBeNull();
    expect(
      projectNextInvoice(
        {
          ...baseMembership,
          plan: { ...baseMembership.plan!, plan_type: 'session_pack' },
        },
        TODAY
      )
    ).toBeNull();
  ...
  it('returns null for an expired member (no phantom past-dated Upcoming)', () => {
    ...
    expect(
      projectNextInvoice({ ...baseMembership, end_date: '2026-01-01' }, TODAY)
    ).toBeNull();
  });

  it('returns null when the current cycle ends exactly today', () => {
    expect(
      projectNextInvoice({ ...baseMembership, end_date: TODAY }, TODAY)
    ).toBeNull();
  });
