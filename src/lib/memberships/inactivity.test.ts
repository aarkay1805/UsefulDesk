...
import { istDayOf, daysSinceVisit, partitionInactivity } from './inactivity';
...
describe('daysSinceVisit', () => {
  ...
  it('counts whole IST days since the last visit', () => {
    expect(
      daysSinceVisit(
        row({ last_visit_at: '2026-07-01T05:00:00Z', visit_count: 1 }),
        today
      )
    ).toBe(4);
  });
  ...
  it('is 0 for a visit earlier today (IST)', () => {
    expect(
      daysSinceVisit(
        row({ last_visit_at: '2026-07-05T03:00:00Z', visit_count: 1 }),
        today
      )
    ).toBe(0);
  });
...
describe('partitionInactivity', () => {
  it('routes members into inactive / neverVisited and drops the recently active', () => {
    const res = partitionInactivity(
      [
        row({
          membership_id: 'recent',
          last_visit_at: '2026-07-03T05:00:00Z',
          visit_count: 5,
        }), // 2d — active
        row({
          membership_id: 'stale',
          last_visit_at: '2026-06-20T05:00:00Z',
          visit_count: 9,
        }), // 15d — inactive
  ...
  it('sorts inactive stalest-first and neverVisited by start date', () => {
    const res = partitionInactivity(
      [
        ...
        row({
          membership_id: 'a',
          last_visit_at: '2026-06-10T05:00:00Z',
          visit_count: 1,
        }),
    ...
    expect(res.neverVisited.map((r) => r.membership_id)).toEqual([
      'old',
      'new',
    ]);
  });
