...
describe('attendanceWindowStart', () => {
  ...
  it("'week' snaps back to the locale week start", () => {
    ...
    expect(attendanceWindowStart('week', { ...base, weekStart: 1 })).toBe(
      '2026-07-06'
    ); // Monday
    expect(attendanceWindowStart('week', { ...base, weekStart: 0 })).toBe(
      '2026-07-05'
    ); // Sunday
    expect(attendanceWindowStart('week', { ...base, weekStart: 6 })).toBe(
      '2026-07-11'
    ); // Saturday = today
  });
});
...
describe('attendanceUsage', () => {
  ...
  it('is unlimited when the plan has no limit or is a session pack', () => {
    expect(
      attendanceUsage(limitedPlan({ attendance_limit_count: null }), 99).limit
    ).toBeNull();
    expect(
      attendanceUsage(limitedPlan({ plan_type: 'session_pack' }), 99).limit
    ).toBeNull();
  });
});
...
describe('checkInWarning', () => {
  ...
  it('never warns for an unlimited plan', () => {
    expect(
      checkInWarning(limitedPlan({ attendance_limit_count: null }), 999)
    ).toBeNull();
  });
});
...
describe('usageSummary', () => {
  ...
  it('returns null for an unlimited plan', () => {
    expect(
      usageSummary(limitedPlan({ attendance_limit_count: null }), 99)
    ).toBeNull();
  });
  ...
  it('stays consistent with checkInWarning at the threshold', () => {
    ...
    for (const used of [11, 12]) {
      ...
      expect(summary?.danger).toBe(
        checkInWarning(limitedPlan(), used) !== null
      );
    }
  });
});
