...
describe('deriveOnboardingSteps', () => {
  ...
  it('requires an active plan with an active pricing option', () => {
    const withoutPricing = deriveOnboardingSteps({
      ...nothingDone,
      hasActivePlanPricing: false,
      whatsappConnected: true,
      templateApproved: true,
    });
    expect(withoutPricing.recommended?.id).toBe('plan');

    const withPricing = deriveOnboardingSteps({
      ...nothingDone,
      hasActivePlanPricing: true,
      whatsappConnected: true,
      templateApproved: true,
    });
    expect(withPricing.recommended?.id).toBe('member');
  });
