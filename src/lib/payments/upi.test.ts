...
describe('buildUpiLink', () => {
  it('builds a full link with amount fixed to two decimals', () => {
    expect(
      ...
      'upi://pay?pa=gym%40ybl&pn=Iron+Fitness&am=1500.00&cu=INR&tn=Renewal'
    );
  });

  it('omits amount (and currency) when not a positive number', () => {
    expect(buildUpiLink({ vpa: 'gym@ybl', amount: 0 })).toBe(
      'upi://pay?pa=gym%40ybl'
    );
    expect(buildUpiLink({ vpa: 'gym@ybl', amount: null })).toBe(
      'upi://pay?pa=gym%40ybl'
    );
    expect(buildUpiLink({ vpa: 'gym@ybl', amount: NaN })).toBe(
      'upi://pay?pa=gym%40ybl'
    );
  });
