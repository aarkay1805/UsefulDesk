    ...
    from(table: string) {
      ...
      const builder = {
        ...
        maybeSingle() {
          ...
          if (table === 'contact_tags' && state.operation === 'delete') {
            return Promise.resolve({
              data:
                options.deleteData === undefined
                  ? { id: 'join-1' }
                  : options.deleteData,
...
describe('removeContactTag', () => {
  it('requires a row to be returned from the destructive write', async () => {
    await expect(removeContactTag(fakeDb(), input)).resolves.toBeUndefined();
    await expect(
      removeContactTag(fakeDb({ deleteData: null }), input)
    ).rejects.toMatchObject({ status: 404 });
  });
