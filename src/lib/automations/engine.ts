...
async function executeAutomation(automation: Automation, input: DispatchInput) {
  ...
  const { data: log, error: logErr } = await db
    ...
    .insert({
      ...
      steps_executed: [],
      // A run is successful only after execution reaches a terminal path.
      // If the process is frozen after this insert, the durable row must not
      // claim that zero executed steps were a completed success.
      status: 'failed',
    })
    .select()
    .single();
...
async function runStep(
  ...
  switch (step.step_type) {
    ...
    case 'create_follow_up': {
      ...
      const dueInDays = Number.isFinite(cfg.due_in_days)
        ? Math.max(0, cfg.due_in_days)
        : 0;
