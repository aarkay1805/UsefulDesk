...
export const SceneBackground: React.FC<{accent?: "violet" | "green" | "amber"}> = ({accent = "violet"}) => {
  const frame = useCurrentFrame();
  const accentColor = accent === "green" ? "rgba(34,197,94,0.22)" : accent === "amber" ? "rgba(245,158,11,0.20)" : "rgba(124,58,237,0.26)";
    ...
    >
      ...
      <Interactive.Div
        ...
        style={{
          ...
          bottom: -400,
          left: -160,
          background: "radial-gradient(circle, rgba(124,58,237,0.15) 0%, rgba(13,13,20,0) 70%)",
      ...
      <div
        style={{
          ...
          inset: 0,
          opacity: 0.14,
          backgroundImage: "radial-gradient(rgba(255,255,255,0.18) 0.7px, transparent 0.7px)",
...
export const Brand: React.FC<{compact?: boolean}> = ({compact = false}) => (
  ...
  >
    ...
    >
      <svg width={compact ? 28 : 35} height={compact ? 28 : 35} viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="2.25" strokeLinecap="round" strokeLinejoin="round">
...
export const Eyebrow: React.FC<{children: React.ReactNode; name: string}> = ({children, name}) => (
  <Interactive.Div
    name={name}
    style={{
      display: "inline-flex",
  ...
  >
    <span style={{width: 8, height: 8, borderRadius: 99, backgroundColor: "#8b5cf6", boxShadow: "0 0 20px #8b5cf6"}} />
    {children}
  </Interactive.Div>
);

export const Avatar: React.FC<{initials: string; tint: string}> = ({initials, tint}) => (
  <div
    style={{
      width: 58,
      height: 58,
