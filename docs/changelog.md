...
## Conflict-resolving member import wizard

Members CSV/XLSX import now retains every source row in one pure candidate collection from Resolve issues through confirm, commit, and a downloadable privacy-safe receipt. Latest parsed start date wins per legacy Member ID; older history, summary rows, and existing members remain visible automatic exclusions. Missing/invalid/shared phones, grouped plan + billing choices, payment disagreements, and existing-contact profile choices must be repaired, resolved, or explicitly excluded before confirm; expiry differences remain notices and the CSV expiry wins. Analysis is deterministic and local-authoritative, and `/api/members/import-analyze` accepts aggregate headers/counts/formats only—never source values. Commit processes included + ready candidates only and retains partial payment failures after member creation. Key code: `src/lib/memberships/member-import-candidates.ts`, `src/components/members/import-members-csv-dialog.tsx`, `src/components/members/import-members-preview.tsx`, `src/lib/memberships/import-commit.ts`, and `src/app/api/members/import-analyze/route.ts`. Boundary: this imports current memberships only; historical rows are reviewable exclusions, not persisted membership history, and no import-job model or migration was added.
