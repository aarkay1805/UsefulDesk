...
## Razorpay Stage 4 full-refund foundation

UsefulDesk now supports admin-only full remaining-payment Razorpay refunds with immutable refund/allocation facts, canonical request bytes and hashes, provider-idempotent ambiguous-create recovery, signed webhook/reconciliation finalization, Dashboard refund import, and separate `reopen_balance` / `reduce_charge` accounting. The latter creates one equal append-only invoice adjustment atomically; external partial refunds reduce net cash, hold the invoice non-collectible, suppress unsafe collection/reminder actions, and remain explicitly unallocated for later line targeting. Key code is `src/lib/payments/razorpay-refunds.ts`, the refund API/routes and dialog, refund-aware Finance loaders/exports, and migrations `20260809165718`–`20260809172816`. Migration `20260809185043` safely extends the service-only, Test-only provider-retry acceptance harness to one exact local refund UUID from signed provider notes. Genuine isolated Test evidence now includes a ₹1.01 UsefulDesk refund recovered after an intentionally ambiguous provider response (`rfnd_TNlm2Bm865srX2`), an imported/classified full refund, a Dashboard-originated ₹1.00 partial refund, and full `reopen_balance` refund `rfnd_TNmPln6l55dKxs`: signed event `TNmQOT5sYfpjpn` first received a controlled 503, then Razorpay redelivered the identical event ID and raw-body SHA-256 1.257 seconds later for a 200/exactly-once finalization. Gotcha: Stage 4 remains **not accepted under the strict rollout gate** solely because the required partial-refund test deliberately leaves one unresolved `partial_refund_line_target_required` exception. OAuth, manual rollback, ambiguity acceptance, and refund-retry acceptance are false on restored READY deployment `dpl_7s1VtoUyXyUb3V2JiPqTCyovxsRj`; no broader or production rollout is authorized.

## Razorpay Stage 3 Payment Links

UsefulDesk can now create or reuse one exact full-balance INR Razorpay Payment Link per eligible invoice revision, copy it without WhatsApp, and settle only from a verified `payment_link.paid` event. The durable `creating | created | cancel_requested | paid | cancelled | expired | orphaned | failed` lifecycle, provider-unique references, seven-day/non-partial contract, recovery worker, service-only settlement/exception RPCs, deterministic generic-invoice allocations, and gateway-payment void guard live in `src/lib/payments/razorpay-payment-links.ts`, the shared webhook/recovery processor, `/api/payments/razorpay/payment-links`, and migrations `20260809140612`–`20260809153500`. The isolated Test acceptance created and paid a mixed ₹1.00 invoice, cancelled an invalidated ₹1.00 revision, then created and paid its ₹1.01 replacement; genuine application-canonical/legacy-shadow events settled each payment once, replay returned the same payment, and the final state had zero active/failed links, unresolved events, or payment/charge exceptions. Gotcha: `gym_payment_link` is not approved, so Send remains disabled while Copy and the complete provider lifecycle remain available; OAuth/manual rollback flags are false on READY deployment `dpl_AKMLBbZUXfRcMKbfuZ7eoK8VpxPs`, and no production/Live or refund work is authorized.
