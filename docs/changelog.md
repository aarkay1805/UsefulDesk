...
Stage 5 now has a deployed, fail-closed production foundation for only account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` and Live merchant `acc_TCJwBqanN9LTrK`. Deployment `dpl_2A4Jf1hf9ZTxZNhex6M17pa1nKkU` enforces Live mode, the exact redirect and pilot id, with OAuth/manual/provider/refund acceptance flags false and no OAuth or application-webhook secret deployed. Production Supabase received Stage 1–4 and migration `20260810160000_razorpay_live_application_ingress.sql` through the approved connector; it adds a service-only, audited, readiness-gated canonical-selector activation. OAuth connect now blocks unreviewed legacy secret material and all non-pilot Live accounts, and the root application webhook rejects Test/Live flag mismatch. The provider-confirmed non-Live legacy key/webhook material was removed from the exact pilot row without reading it; no manual Live key was created. The owner explicitly accepted temporary use of the existing unrotated OAuth client secrets and asked ticket `20303463` to close with no rotation or application change. Gotcha: the authenticated Razorpay session expired before the exact 16-event Live webhook and secret-blind OAuth credential entry, so no Live OAuth authorization, signed delivery, provider probe, or financial mutation exists and Stage 5 is not accepted. Key code is `src/lib/payments/credentials.ts`, the OAuth/application-webhook routes, and the new migration; operational evidence and the remaining gate are in `docs/razorpay-operations.md`.
