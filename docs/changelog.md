...
## Razorpay P2 integrity gaps are closed in the repository

Application-canonical webhooks now claim signed unknown merchants into `webhook_events` with a null tenant instead of stopping at delivery observation; the event stays failed/unprocessed until the recovery worker can bind the exact OAuth merchant + mode. AutoPay and Payment Link settlements now require Razorpay's provider-authored payment timestamp and stamp both ledger/exception state through service-only, identity-checked RPCs, so reporting and refund-age checks no longer use webhook processing time. Owner/admin users can cancel a live member mandate from Billing with a required reason: the server fetches the exact subscription, cancels immediately, re-reads after an ambiguous response, and changes UsefulDesk to audited terminal/manual state only after Razorpay confirms `cancelled`, `completed`, or `expired`. Mandate setup remains same-origin, now requires the exact `active` status (including rejecting stored `expired` rows), and fails closed when account currency cannot be loaded. Key code lives in the mandate and application-webhook routes, `src/lib/payments/razorpay-{webhook-processor,payment-links,mandates}.ts`, the member detail Billing UI, and migration `20260826210000_resolve_razorpay_p2_integrity_gaps.sql`. Gotcha: the migration is forward-only and has not been applied by this local pass; deploy it through the approved Supabase migration tool before deploying the dependent application code, and validate cancellation only in Razorpay Test Mode before Live use.
