...
## Razorpay OAuth Stage 1 acceptance and Stage 2 shadow ledger

The isolated Razorpay Test stack now has real development-client S256 authorization/code exchange evidence, five-capability imported-account readiness fallback, one successful lease-protected refresh rotation, and verified disconnect token/merchant scrubbing; both rollout flags were restored false afterward. Stage 2 adds the service-only `razorpay_webhook_deliveries` ledger and parity view, shared header/hash event identity, current/previous application-secret tracking, durable application shadow observations with a database-enforced zero-mutation invariant, and legacy observations that stop before canonical claims after cutover. Key code: migration `20260809100000`, `src/lib/payments/razorpay-webhook-delivery.ts`, and both Razorpay webhook routes. Gotcha: the ledger has no real dual-ingress pair yet, and a CLI-pulled-secret probe did not match the deployed application secret; authenticated Razorpay/Vercel secret review, AutoPay mutation acceptance, recovery scans, and cutover remain blocked. The application route returns 503 if the selector is flipped prematurely.
