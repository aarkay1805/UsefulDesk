...
UsefulDesk now supports admin-only full remaining-payment Razorpay refunds with immutable refund/allocation facts, canonical request bytes and hashes, provider-idempotent ambiguous-create recovery, signed webhook/reconciliation finalization, Dashboard refund import, and separate `reopen_balance` / `reduce_charge` accounting. The latter creates one equal append-only invoice adjustment atomically; external partial refunds reduce net cash, hold the invoice non-collectible, suppress unsafe collection/reminder actions, and remain explicitly unallocated for later line targeting. Key code is `src/lib/payments/razorpay-refunds.ts`, the refund API/routes and dialog, refund-aware Finance loaders/exports, and migrations `20260809165718`–`20260809172816`. Genuine isolated Test evidence includes a ₹1.01 UsefulDesk refund recovered after an intentionally ambiguous provider response (`rfnd_TNlm2Bm865srX2`), an imported/classified full refund, and a Dashboard-originated ₹1.00 partial refund. Gotcha: Stage 4 is implemented and provider-exercised but is **not accepted under the strict rollout gate** because the required partial-refund test deliberately leaves one unresolved `partial_refund_line_target_required` exception and Razorpay did not redeliver the Stage 4 refund event for genuine duplicate evidence. OAuth and the acceptance-only ambiguity flag are false on READY deployment `dpl_JB2XKxvf7MBqUJ53u9ajMSaDJisF`; no broader or production rollout is authorized.
