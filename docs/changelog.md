...
## Razorpay Stage 5 production preflight blocked before live configuration

Stage 5 began with a read-only production capability and isolation inventory. Production Vercel project `useful-desk` (`prj_kn3FOeuAZkeAyCeA5lbBhsHHECne`) is READY at deployment `dpl_DQ89zFgKbfJmC66ENPJtxwBL94na`, but has no production `RAZORPAY_*` environment variables. Production Supabase project `fwqthstqrkrwtaehefks` is distinct from the accepted Test project and still ends at the recurring-charge migration; its one configured Razorpay row uses the legacy unversioned manual-secret schema, has no reviewed provider mode or external merchant mapping, and belongs to account `50a9e8f9-d7e5-44d2-ba04-c367509b981e`. The Razorpay application has the production client and correct HTTPS callback, but no live application webhook; live event availability, Payment Links, Subscriptions/Recurring, Payments/refunds, merchant authorization, and real-money choices are unverified. No provider, database, Vercel, flag, credential, or financial state changed. Stage 5 remains blocked until the pilot branch/merchant and legacy-row mode are explicitly confirmed, the exact acceptance-credential rotation scope is authorized, and the later payer/amount/refund choices are supplied. Key evidence is in `docs/razorpay-operations.md`; do not apply the Stage 1–4 migrations, backfill secrets, create the live webhook, rotate credentials, or authorize a merchant by inference.
