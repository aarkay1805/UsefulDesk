...
## Razorpay Stage 5 pilot and Live capability preflight

Stage 5 now has an explicitly selected pilot: production account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` (`Rajat Kashyap`) maps to Razorpay Live merchant `acc_TCJwBqanN9LTrK`, which is activated and not yet transacted. Live Dashboard checks prove Payment Links and Subscriptions are available, and the unsaved application-webhook selector exposes the same 16 consumed account/subscription/link/refund events used in Test. The Live merchant has no API key, proving the production database's old unversioned manual row is not a Live credential and must never be reused or inferred from its prefix. Production Vercel still has no `RAZORPAY_*` variables, production Supabase still lacks the Stage 1–4 migrations, and no live webhook exists. The next hard gate is provider-controlled credential rotation: the application UI exposes both OAuth client secrets only as reveal-only disabled fields and publishes no self-service rotation action. Razorpay support ticket `20303463` now requests rotation of both the Development and Production OAuth client secrets while preserving the application, client IDs, redirect URIs, and webhook configuration; Razorpay states a 4–8 business-hour update window. No credential, provider configuration, database, Vercel, flag, webhook, merchant authorization, or financial state changed. Rotation must complete before Live OAuth authorization; only then may configuration, migration, connection, API/refund probes, or money proceed. Key evidence is in `docs/razorpay-operations.md`.
