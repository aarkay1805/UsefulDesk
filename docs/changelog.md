...
is complete and now omits **Configure** at the same boundary, leaving one clear
**Set up message** action. Once ready, the row reveals both the switch and the
**Configure / Hide configuration** disclosure. Setup stays reachable with
permission explanations, and installments stay payment-plan managed without a
switch. Setup navigation never PATCHes rule activation.
...
recommended patches, declares the font/image config plugins, regenerates typed
routes before typecheck, and excludes ignored Expo/native output from Prettier.
The mobile start, iOS, and Android scripts also expose the workspace-local
module path so Expo's hoisted CLI can initialize typed routes instead of exiting
on `expo-router/_ctx-shared` resolution.
Shared mobile small buttons, icon buttons, composer fields, and text fields now
use a 48pt minimum target across iOS and Android. Expo Doctor passes 21/21; a
local iOS development client built and installed on the paired iPhone, and
Android assembled a debug APK. After the iPhone was unlocked, the development
client launched through Metro and passed branch isolation, foreground/session
recovery, persisted failed-row safety, and base light/dark appearance. The
maximum-standard Dynamic Type defect now has a passing dark-appearance physical
retest: `inbox-layout.ts` switches message bubbles to an 88%-wide reflow,
timestamps and delivery state leave the body line, conversation rows keep
untruncated identity/preview copy with metadata below, and native text slots use
content-driven line metrics. The Account header action is now a fixed SF Symbol
in the same 48pt rounded-rectangle target. The full template body, date/status
metadata, failed state, composer, filters, and row content are visible without
overlap. At the same maximum Text Size in light appearance, the Inbox and
Account surfaces also remain readable without clipping or overlap. A
`__DEV__`-only exact deep-link fixture in `inbox-test-fixtures.ts` and
`conversation-screen.tsx` now injects in-memory repositories, a no-op realtime
feed, and a sender that throws before transport. It reuses the production
conversation/message/composer rendering without creating a customer record or
contacting anyone. The physical iPhone light-mode maximum-Text-Size retest now
passes the header, date separator, multi-line inbound/outbound bubbles,
timestamps/read ticks, Message field, and Send target with no clipping or
overlap; no text was entered and Send was not pressed. VoiceOver transition
speech, viewer mode, iOS edge-swipe navigation, live inbound realtime, and both
remote EAS builds remain unverified. Build/acceptance commands and the no-secret
recording boundary live in `docs/mobile/development-build.md`. Root
ESLint now excludes tracked agent-tool packages and ignored sibling worktrees
from the application lint boundary.
...
Approved template was submitted to an approved test contact: the API returned
200 with a Meta provider ID, but callback code `131049` rejected delivery for
healthy-ecosystem engagement, so the test contact did not receive it. The final no-resend
...
534 tests, root lint with 0 errors / 3 existing warnings, root typecheck, all
406 root suites / 3,055 tests, the 66-test bearer/send/proxy selection, and a
fresh Next production build plus iOS and Android exports. Expo Doctor passes
21/21 after the recommended SDK patch upgrade. A free-form send, local
optimistic text Retry, physical viewer/VoiceOver transition behavior, and live
realtime were not accepted.
Base light mode was exercised before the reflow fix; the post-fix maximum-size
dark pass is recorded above, and the matching light pass now covers Inbox and
Account plus the fail-closed local conversation/composer fixture.

Physical Android 11 acceptance on a OnePlus 6 now covers main-checkout
build/install/launch, authenticated session restoration, Inbox filters/search,
system Back, branch isolation, and foreground recovery. The branch chooser now
guards cleanup across branch-keyed unmounts; the conversation screen keeps the
composer above Gboard, re-pins a bottom-following conversation when its viewport
resizes, and stabilizes that keyboard offset around runtime banners.
The normal-font synthetic fixture passed keyboard, no-send, Back, and focused
log checks, and device settings were restored to night `auto` / font scale
`1.0`. A pre-repair light/1.3 run lost the newest bubble; the repair passes
automated tests but the user deferred its post-fix physical large-font,
light/dark visibility, accessibility, and screenshot checks. Android 12+
theming, predictive Back, tablet/foldable layouts, provider delivery, and remote
EAS remain unverified. The root `npm run verify` still stops at one unchanged
tracked Prettier baseline;
...
The owner's pending AutoPay mandate exposed two independent failures. Immediate
...
## Dashboard action widgets no longer fan out through the browser

Gym metrics, follow-ups, expiring memberships, uncontacted leads, and needs-attention now hydrate through one branch-authorized, viewer-readable, no-store **bounded dashboard action snapshot** at `src/app/api/dashboard/actions/route.ts`. The action slice falls from 14 browser data requests to one (13 fewer, 92.86%): follow-up assignees are limited to the users referenced by the bounded rows, both eight-row follow-up scopes are preloaded, expiring and uncontacted previews stop at eight rows, and message previews stop at 160 characters. Section failures remain independent, post-mutation refreshes reuse the boundary, and WhatsApp readiness is deferred until a member detail opens. Gotcha: keep the endpoint RLS-scoped and branch-calendar-authoritative; the separate insights snapshot and the server-side database fan-out remain deliberate boundaries.

## Dashboard insights no longer fan out through the browser

The historical dashboard cards now hydrate through one branch-authorized, no-store **bounded dashboard insights snapshot** at `src/app/api/dashboard/insights/route.ts`. The initial browser path falls from at least 16 Supabase data requests to one API request (15 fewer, 93.75% for this slice); conversation and lead-rating range changes use the same boundary, so raw message/contact/membership/follow-up histories no longer enter browser JavaScript. `src/lib/dashboard/insights-snapshot.ts` keeps failures section-local and caps the activity preview at 50, while `src/lib/dashboard/queries.ts` preserves calendar behavior with the selected branch timezone. Gotcha: keep this endpoint viewer-readable, RLS-scoped, branch-authoritative, and uncached; it replaces existing reads rather than introducing an admin-only or stale analytics path.

## Member import Resolve issues is a real two-pane workspace

The Resolve issues step now gives the queue rail and the focused issue one scrollport each; the step frame itself no longer scrolls, so the tab strip and the ready/excluded tally stay pinned while either pane moves. The sizing had to become flex all the way down (`import-members-csv-dialog.tsx` body → `motion.div` → `Tabs` root → `TabsContent` → the two-pane grid). Gotcha: `h-full` cannot do this job here — `DialogContent` is `max-height` clamped rather than height-set, so a percentage height resolves against an indefinite parent, collapses to `auto`, and drops the whole step into one shared column scroll. Layout follow-on: the body drops its padding at this step, so the tab strip, the rail, and the working pane carry the dialog's own 24px gutter and the rail divider runs full height; `sm:max-w-[1200px]` gained a `min(...)` so the dialog keeps its 16px viewport margin between 640px and 1232px; and `IssueRows` uses a container query on the focused-issue section instead of `sm:`, because the fixed-width rail leaves that column far narrower than the viewport breakpoint implies. The dialog header was reworked in the same pass, because both defects only showed at phone width: the step indicator names only the step you are on below `sm` (the numbered circles still show the whole flow and the other names stay in the accessibility tree) instead of scrolling `Confirm` off the end behind `overflow-x-auto`, and the draft bar no longer wraps `Start fresh` onto an orphaned second row — it is one line at every width, the connector word `Continuing` is dropped below `sm` so the filename itself gets the space, and the filename carries a `title`. That returns roughly 28px to the panes below. Key code: `src/components/members/import-members-preview.tsx` and `src/components/members/import-members-csv-dialog.tsx`.

## Dashboard cold start and onboarding fan-out are bounded

The authenticated app now enters through a server-hydrated dashboard bootstrap in `src/app/(dashboard)/layout.tsx` and `src/lib/auth/dashboard-bootstrap.ts`: the canonical server `getUser()` validation loads profile, branch access, optional appearance, and selected-account context before hydration, while `AuthProvider` retains its browser auth-state listener for refresh/sign-out without repeating the cold `getSession()` and four-read waterfall. The proxy uses signed claims for its early redirect and forwards only the URL-derived branch header upstream; membership and account checks remain fail-closed. Active onboarding now calls one settings-authorized `/api/onboarding/status` endpoint instead of eight browser endpoints, with the same incomplete-on-failure semantics. Gotcha: do not remove the server `getUser()` boundary or publish account context when the selected account lookup fails.

## Production workers now have an independent database-owned scheduler

Supabase Cron now calls one Vault-authenticated application aggregator for the seven operational workers every 15 minutes and the two reminder workers hourly. The database generates and retains its own 256-bit secret in Vault; only a service-role RPC can verify its SHA-256 digest, and the aggregator delegates to the unchanged `AUTOMATION_CRON_SECRET` boundaries so every existing claim, dedupe, failure response, and GitHub diagnostic remains authoritative. GitHub Actions stays enabled on staggered off-boundary minutes as the redundant pinger and alert path after its scheduled events exceeded the runbook freshness limits on 2026-08-27. Manual recovery runs `33046275176`, `33046277318`, and `33046279868` were green, then Production database requests `1` and `2` returned HTTP 200 with all seven ops and both reminder routes successful: no reminders were due or sent, every recovery queue was empty/healthy, and every aggregate failure count was zero. Connector migrations `20260827064753`, `20260827070049`, and `20260827070239` installed, hardened, and activated both jobs after commit `f81242a` passed CI and reached Production; the first natural ops tick at `2026-08-27T07:08:00Z` succeeded and pg_net response `3` returned HTTP 200 with seven dispatched and zero failed. Key code: `src/app/api/database-cron/route.ts`, migrations `20260827064004_database_owned_cron_scheduler.sql`, `20260827070010_harden_database_cron_verifier.sql`, and `20260827070201_activate_database_owned_cron_scheduler.sql`, the three production workflows, and `docs/automations-and-cron.md`. Gotcha: never expose the Vault secret or replace the aggregator with direct database access to provider logic.
...
Four pre-OAuth Razorpay subscription events were received under the owner's retired manual webhook while their immutable subscription notes named the VBF tenant. Read-only review proved all four were Test-mode events for merchant `acc_TCJwBqanN9LTrK`, predated Live application-webhook activation, referenced no surviving member/contact/mandate, and had zero matching payment, allocation, refund, exception, or delivery effect. Migration `20260826070646_reconcile_legacy_razorpay_identity_mismatches.sql`, connector-applied to Production as `20260826071158`, adds an immutable service-only audit and a Test-only terminal reconciliation RPC that rechecks every identity/object/zero-effect invariant atomically. The exact four rows now retain their receipt tenant and raw payload, carry the proven Test mode and external merchant, preserve null legacy signature-source/hash facts, and are processed with `legacy_identity_mismatch_no_local_effect`; no webhook was replayed and no ledger or membership row changed. Production now has zero failed or unprocessed Razorpay events, while the pinned Live OAuth connection and its zero failed/missing-ledger scope remain unchanged.

## Provider-discovered Razorpay charges now have an audited resolution path

Settings → Payments now identifies the affected member, amount, captured time, and payment suffix for a `provider_charge_missing_webhook`. An admin can **Apply to membership**, which freshly rechecks the Razorpay subscription, paid invoice, captured payment, and zero-refund state before an atomic service-only replay through `record_gateway_charge`, or **Mark handled externally**, which records the actor/reason without creating any ledger credit. The apply path accepts only the immediately next `paid_count`, preserves Razorpay's payment time, and leaves a newly diagnosed invariant failure open instead of forcing money into the wrong period. Code lives in `src/lib/payments/razorpay-charge-resolution.ts`, the charge-exception route, Settings card, and migration `20260826043822_resolve_razorpay_provider_charge_exceptions.sql`. Production records connector version `20260826044721`; service-only grants, actor guard, canonical write, paid-time preservation, unchanged zero-open-exception state, and advisors were verified. No resolution function ran and no payment or membership changed during migration.

## Razorpay recurring-charge recovery now repairs ordering and detects missing webhooks

The recovery worker now replays only the immediately next `charge_sequence_mismatch` under an owner lease and resolves its durable exception in the same transaction as the ledger write, so an early `subscription.charged` delivery no longer remains permanently stranded after its predecessor arrives. A separate bounded daily provider-source scan fetches each subscription plus its complete paid-invoice history, verifies every missing invoice against the captured payment, and preserves missing-webhook money as a review-held `provider_charge_missing_webhook` exception without auto-crediting the ledger. Key code lives in `src/lib/payments/razorpay-recovery.ts`, `src/lib/payments/razorpay-subscription-reconciliation.ts`, and migrations `20260826033801_razorpay_recurring_charge_recovery.sql` plus `20260826034807_razorpay_subscription_source_reconciliation.sql`. Production records connector-applied versions `20260826040518` and `20260826040539`; columns, indexes, service-only grants, review-only function definition, resource counts, and Supabase advisors were verified before the application release. The first naturally scheduled Production Live scan later claimed and scanned the one due mandate with zero observations and zero failures.

## Razorpay subscription containment closes the first audit risks

The first audit-fix slice separates transient Razorpay subscription status from UsefulDesk mandate lifecycle, so `subscription.pending` records provider retry state without failing the mandate while `halted` remains terminal. Signed OAuth authorization revocation now clears unusable grants and requires reconnect without erasing the merchant mapping; the daily recovery scan re-verifies stale imported-account readiness; disconnect atomically refuses active mandates, links, refunds, unresolved webhooks, and open payment/refund exceptions with an actionable conflict; and mandate setup now enforces same-origin, active membership, and authoritative account currency before provider access. Key code lives in `src/lib/payments/razorpay-webhook-processor.ts`, `src/lib/payments/credentials.ts`, `src/lib/payments/razorpay-recovery.ts`, and the mandate/disconnect routes; migration `20260825191937_razorpay_subscription_containment.sql` is additive and service-role-only. Production records connector-applied version `20260826040422`. Its first application omitted the file's trailing revokes during packaging, so immediate forward-only version `20260826040453` and repository migration `20260826040638_harden_razorpay_subscription_containment_grants.sql` reapplied every service-only grant before application deployment; privilege queries verified `anon` and `authenticated` cannot execute the RPCs. No mandate, link, refund, payment, membership, or credential state changed. New AutoPay setup remains enabled by owner decision.

## Razorpay reconnect recovery now handles a blocked fresh grant

The owner's pinned Live merchant was reconnected after its prior OAuth row had been explicitly disconnected. Razorpay's Accounts readiness endpoint rejected the imported merchant even though authenticated customer, plan, subscription, payment-link, and payment probes all succeeded; the callback therefore stored a fresh grant as `blocked`, while the Verify route could only resolve `ready` rows and returned an internal error. The recovery path in `src/lib/payments/credentials.ts` and `src/app/api/payments/razorpay/oauth/refresh/route.ts` now admits a blocked grant only for explicit read-only readiness verification, without weakening ordinary payment access; ready rows still force token refresh. `src/components/settings/razorpay-settings-card.tsx` also renders a truly disconnected OAuth row as one reconnect task instead of contradictory ready/live badges and stale Verify/Disconnect actions. Regression tests cover both boundaries. Production was restored to OAuth/Live/ready for exact merchant `acc_TCJwBqanN9LTrK` after all five capability probes returned 200, with zero failed webhooks, missing ledger rows, open payment exceptions, or stuck links. No payment, link, refund, message, or other money movement was created.
...
## Razorpay Live rollout authority now supports the owner account and VBF safely

The one-account environment pins are replaced by the browser-denied
`razorpay_live_rollout_accounts` table. The owner account stays exactly bound; VBF account
...
check proved the owner-approved production account now stores the exact
...
Vercel's one-hour Hobby runtime-log window, exact alert thresholds, the owner's
...
back to the server-side code exchange, accepts only the Meta SDK callback or
the current HTTPS origin, and keeps review-tunnel CSRF exceptions limited to
the configured non-production forwarded host. Page lead-access parsing accepts
Meta's current direct response shape; Pages for which Meta withholds the
optional diagnostic can still prove and repair their `leadgen` subscription
without recording a false lead-access verification. The Page webhook handshake
prefers `META_LEADGEN_VERIFY_TOKEN` and otherwise derives a stable,
domain-separated token from the existing app secret. Migration
`20260823100000_preserve_unverified_meta_lead_access.sql` is applied to
UsefulDesk Production. Key code: `src/components/settings/meta-leads-connect.tsx`,
`src/app/api/meta/leads/connect/route.ts`, `src/lib/meta/lead-ads-health.ts`,
`src/lib/meta/webhook-verify-token.ts`, and `src/lib/whatsapp/meta-api.ts`.
Gotcha: an unavailable `has_lead_access` diagnostic is not treated as an
approval; `lead_access_verified_at` remains unchanged while the independently
verified Page subscription stays healthy. Meta's test tool can also send its
...
Meta Lead Ads now shares one tenant-owned ingestion processor between the signed webhook and a bounded recovery worker, and checks due Page connections with exact lead-access diagnostics before repairing a missing `leadgen` subscription. Migration `20260822100000_meta_lead_ads_self_healing.sql` adds credential generations, Page/event leases, retry schedules, atomic phone-less completion, service-role-only RPCs, and one generic admin/owner notification per active incident; `20260822100001_index_meta_page_config_user_id.sql` covers the nullable audit-user foreign key identified by post-DDL advisors. Settings exposes localized health, Check now, reconnect, and disconnect actions; `/api/meta/leads/recovery/cron` is scheduled every 15 minutes after event recovery, with aggregate non-PII output. Key code: `src/lib/meta/`, `src/app/api/meta/leads/`, and `src/components/settings/meta-leads-connect.tsx`. Production records both migrations as `20260822093418_meta_lead_ads_self_healing` and `20260822093424_index_meta_page_config_user_id`; live verification found every expected column/index/RPC, no browser-role RPC grants, all eight service-role grants, zero configured Pages or queued Meta events, and successful scheduled recovery on deployment commit `e13098e`. The **Capture & manage ad leads with Marketing API** use case is attached to the live Meta app and draft `1914379289558468` is scoped to `pages_show_list`, `pages_manage_metadata`, `leads_retrieval`, and Meta's required `pages_manage_ads`, `pages_read_engagement`, and `ads_management` dependencies; unused `ads_read` and Marketing API Access Tier requests were removed. The four existing WhatsApp/public-profile renewal allowed-use certifications, all six new permission descriptions/agreements, the data-handling assessment, and reviewer instructions are complete; `docs/meta-data-handling.md` holds the canonical answers and reviewer walkthrough. The reviewer copy retains the existing test credentials, covers both WhatsApp and Lead Ads, and keeps consumer Facebook Login marked **No** because UsefulDesk uses Facebook Login for Business only to authorize selected business assets. The separate **UsefulDesk Lead Ads** Facebook Login for Business configuration (`1039026725782445`) uses the General/user-token flow with exactly those six scopes and does not modify UsefulDesk sign-in or the WhatsApp configuration. Disposable Page **UsefulDesk Lead Ads Test** (Page asset ID `1300231026509095`; profile URL ID `61593607592072`) was created without public content, contact data, branding, WhatsApp linking, or invited followers. Active Instant Form **UsefulDesk Lead Ads Test Form** (`2157208975142771`) collects full name, phone number, and email, links the live UsefulDesk privacy policy, and has no ad or campaign. Temporary branch `codex/meta-leads-app-review` injects the public configuration ID only when Vercel builds that exact ref as a Preview; Production remains dark. Meta's optional `has_lead_access` diagnostic is unavailable for this Page, but the official testing tool confirms user lead permission, Lead Access Manager, Page administration, and clean app diagnostics. The Page is connected, both `leadgen` subscription layers are configured, and the app-level callback uses the stable Production endpoint; its verification handshake and signed sample both succeed. A real synthetic delivery reached Production, and local recovery proved that Meta's nonnumeric dummy-phone placeholder now completes through the phone-less path without creating customer data. A fresh delivery after deployment, the five screencast requirements, and four API test calls remain incomplete; disposable canaries, explicitly authorized Production canary, and dark-launch activation remain pending.

---

## Meta template sync is authoritative without erasing history

A complete Settings → Message templates sync now reconciles both sides of the provider catalogue. Meta-returned rows replace cached status/components and clear stale local submission failures; non-Rejected states also clear obsolete rejection text. Provider-backed rows absent from the complete Meta snapshot are retained for audit, marked `DISABLED`, and shown as **Not on Meta**, so a deleted or moved template can no longer keep a stale Approved badge or enter any Approved-only send path. Incomplete/pagination-capped snapshots never mark rows missing. Reappearing rows recover automatically on the next complete sync, and an already-missing row deletes locally without a doomed Meta delete call. Database-issued `provider_sync_generation` values advance for both present and missing outcomes and supply a total-order compare-and-set guard, so an older overlapping snapshot cannot overwrite a newer reconciliation. Template identity is unique per account and insert races retry as guarded updates; the compatibility migrations report legacy teammate duplicates without deleting their history and redirect the previously deployed user-scoped upsert to the canonical tenant row. Key code: `src/lib/whatsapp/template-sync.ts`, `src/app/api/whatsapp/templates/sync/route.ts`, `src/components/settings/template-manager.tsx`, and the `2026082207…` migrations. Gotcha: `provider_missing_since` is local reconciliation evidence; the database rejects `APPROVED` while that marker is set, and `DISABLED` remains the fail-closed status consumed by existing send selectors. The legacy user-scoped upsert index is retained only for mixed-version compatibility and should be removed after the account-scoped submit route is deployed.
...
**Sequenced deliberately.** Meta needs App Review for the complete provider-required Lead Ads scope: `pages_show_list`, `pages_manage_metadata`, `leads_retrieval`, `pages_manage_ads`, `pages_read_engagement`, and `ads_management` (weeks; resubmitting can re-queue the already-approved WhatsApp permissions). The last three are install/retrieval dependencies; UsefulDesk does not expose ad-management controls. Forms have no such gate, so forms ship live and the Meta path sits **dark behind an unset `NEXT_PUBLIC_META_LEADS_CONFIG_ID`** — the card doesn't render until review clears.
