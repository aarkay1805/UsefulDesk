...
## Razorpay Stage 5 production foundation

Stage 5 now has a deployed, fail-closed production foundation for only account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` and Live merchant `acc_TCJwBqanN9LTrK`. Deployment `dpl_CJuMiLV5EW5VhYpCogGcK6YS248v` enforces Live mode, the exact redirect and pilot id, with OAuth/manual/provider/refund acceptance flags false. Sensitive Production OAuth credentials and a new sensitive application-webhook secret are Production-only; Razorpay has one Live root webhook whose reopened editor confirmed exactly the required 16 events, and invalid signatures return 400. Production Supabase received Stage 1–4 and migration `20260810160000_razorpay_live_application_ingress.sql` through the approved connector; it adds a service-only, audited, readiness-gated canonical-selector activation. OAuth connect blocks unreviewed legacy secret material and all non-pilot Live accounts. The provider-confirmed non-Live legacy key/webhook material was removed from the exact pilot row without reading it; no manual Live key was created. The owner explicitly accepted temporary use of the existing unrotated OAuth client secrets and asked ticket `20303463` to close with no rotation or application change. Gotcha: a shortest OAuth window stopped before Connect because the active UsefulDesk browser session belonged to workspace Aakash Mishra, not the allowlisted Rajat Kashyap pilot; the flag was immediately restored false with no OAuth state, provider grant, selector change, or financial mutation. Stage 5 remains unaccepted. Key code is `src/lib/payments/credentials.ts`, the OAuth/application-webhook routes, and the new migration; operational evidence and the remaining gate are in `docs/razorpay-operations.md`.
