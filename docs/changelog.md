...
Stage 5 now has exact-pilot Live OAuth readiness for account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` and merchant `acc_TCJwBqanN9LTrK`. The callback additionally pins the exchanged merchant through `RAZORPAY_LIVE_PILOT_MERCHANT_ID` and revokes both tokens on mismatch. The real `read_write` grant passed five non-mutating capabilities, persisted one encrypted ready/manual-free connection, and atomically selected application ingress with one immutable audit. Migration `20260810162750` adds provider mode/merchant identity to the read-only missing-ledger view; Settings now scopes failed/missing-ledger health to that exact identity, preserving 16 older null-identity legacy events without contaminating the Live pilot. Live event/failure/missing-ledger/exception/link/refund counts are zero, and deployment `dpl_8ur1o5njFRfm66Qu8X88cY6svL6U` restores every rollout/acceptance flag false. The UI shows Connected, Readiness verified, Live mode, and no alert; invalid signatures return 400. Existing OAuth secrets remain unrotated under the recorded owner risk acceptance. Stage 5 remains unaccepted until the separately authorized low-value Payment Link/capture/full-refund, signed/replay, reconciliation, and finance-surface exercise. Key code is `src/lib/payments/razorpay-config.ts`, `src/lib/payments/credentials.ts`, the connection/OAuth routes, and migrations `20260810160000`/`20260810162750`; operational evidence and the exact money gate are in `docs/razorpay-operations.md`.
