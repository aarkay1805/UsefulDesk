...
- **SEV-3:** one redundant GitHub worker schedule exceeds its 75-minute ops or
  two-hour renewals threshold while the matching database job remains healthy;
  one transient probe fails and its retry succeeds; or another noncritical
...
(two hours), and nightly-backup (30 hours) freshness limits into explicit
Actions annotations. Stale redundant ops or renewals schedules warn without
failing `production-health`; stale backup history fails it because no second
scheduler creates the recovery point. It cannot report while GitHub's scheduler
