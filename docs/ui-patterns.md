...
- **A queue row is a bounded record, not a full-bleed span.** A list row whose subject sits at the left edge and whose state sits at the right edge by `ml-auto` stops being one record the moment the container is wide: on the dashboard the follow-up queue put those ends ~965px apart with nothing between them, which is a reading cost at 100% zoom and two separate rows at 200%. Cap the section — `max-w-3xl` on `DashboardSection`'s `className`, which bounds the **See all** link with it — rather than letting a single-column list inherit the page width. Full bleed is earned by blocks that subdivide into peers (`gym-metrics`, `quick-actions`, `needs-attention-card`), never by one stretched row.
- **Row meta must align into real columns.** A right-anchored `flex` cluster is only as wide as its own content, so `Lead`+`Overdue` and `Member`+`22 Feb 2027` start at different x and no two rows agree where the state column is — the queue cannot be scanned downward for what is late. Give the list the tracks (`sm:grid-cols-[minmax(0,1fr)_auto_auto_auto_auto] sm:gap-x-2.5`) and have each row re-enter them with `col-span-full grid grid-cols-subgrid`, the same way `lead-funnel.tsx` aligns its stage rows. Keep the tracks `auto`, never fixed widths: the due cell holds a badge or a **localized** medium date, so only content sizing survives a change of account locale. Where a column is conditional (the follow-up queue's Lead/Member tag renders only under the All chip), declare a second template minus that track instead of leaving an empty one to pay for a gap. Below `sm` there is no slack to distribute — keep the wrapping flex line and dissolve the mobile cluster wrapper with `sm:contents` so its children become real cells.
