...
- **The title row wraps; it never overlaps.** Row 1 is `flex-wrap` with `min-h-14`, and the actions slot carries `ml-auto`, so when a page's actions do not fit beside the title on a phone they drop to a right-aligned second line. Keep secondary ghost actions (Import, Export, Download) icon-only below `sm` — icon plus `<span className="hidden sm:inline">Label</span>` and an `aria-label` with the same word — so the common pages still fit on one line at 375px. The primary action keeps its label.
...
- **Name a section by the action, not by who it is about.** Home (`/dashboard`) split follow-ups into an enquiry work queue and a member work queue; an owner clearing follow-ups wants one list in due order, not two lists to reconcile. Audience is a filter (`Chip`), not a section boundary.
...
- `DashboardSection` owns the heading row: `min-h-6` reserves the height of a `size="xs"` link so sections with and without a trailing action leave identical space before their content, and its `meta` slot (a total, a help trigger) emits the `{' '}` a flex container drops visually but the accessible name needs (else a section announces "Enquiries by stage1 total"). Trailing links — **See all**, **Open chats** — belong in `action`, beside the heading, not in the card. Pass `className` for external layout only — a section that shares a grid row passes the exported `DASHBOARD_PAIRED_SECTION` (`flex flex-col max-h-[480px]`) and gives its card `min-h-0 flex-1`, which is how sibling blocks sit side by side at equal height **without** a wrapper heading over them.
- **A paired section is capped, and the overflow goes inside the card — never to the page.** Two queues on one row are only readable while the row is one screen object: a follow-up row with a long note is twice the height of a lead row, so an uncapped pair leaves one card short of its neighbour and pushes the next row of work off the fold. Cap the section with `DASHBOARD_PAIRED_SECTION` and give the card's body the canonical `ScrollArea` at `DASHBOARD_QUEUE_SCROLLER` (`min-h-0 flex-1` — a flex child defaults to `min-height:auto`, so without it the scroller grows to fit every row and the card clips with no scrollbar at all, the trap already documented on the inbox conversation list). The `ScrollArea` is a sibling of `CardHeader` and `CardFooter`, never inside them: a chip header and a **Show more** footer must stay put while the rows move under them. Never hand-roll `overflow-y-auto` here.
- Truncation is `QueueCount` (`components/dashboard/action-queue.tsx`): a right-aligned `N of M` that renders **only when the list is truncated**, because a count printed above the rows it counts restates them. Where filter Chips already carry live counts, the section does not repeat them. Give a section a **See all** only where a page owns that exact queue — `/leads` routes just `all | followups`, so the dashboard's uncontacted-enquiry queue correctly has none, and the merged follow-up queue shows one only once an **Enquiries** or **Members** chip narrows it to a queue a page owns.
- **A full-width queue row spends its width on a column, never on a gutter.** A row whose subject sits at the left edge and whose state sits at the right edge by `ml-auto` stops being one record the moment the container is wide: the dashboard follow-up queue put those ends ~965px apart with nothing between them, a reading cost at 100% zoom and two disconnected halves at 200%. The fix is not to cap the section — it is to promote the row's longest field out of a subtitle into its own flexible track, the way a mail list gives sender, subject, and date each a column. Pick the field with enough content to carry the width (the follow-up note; a message preview); everything else gets a content-sized track. If no field can carry it, the row does not belong at full width.
- **A row that no longer has that width takes the column back out.** The same follow-up queue moved into a half-width paired section, where a 1440px window leaves it ~568px. Six tracks do not fit that: beside the four content-sized meta cells the note kept ~90px at a 1280px window, so the field promoted to carry the width stopped carrying anything. It went back under the name as one flexible cell taking the whole remaining width, and only the meta stayed in columns. **Set the container-query threshold from the width the layout actually hands the component, not from the width its row template would like.** `@2xl` (672px) read as a safe "desktop" number and was one this column never reaches until a ~1650px window; every real laptop rendered the phone fallback at 97px a row, with four of eight visible inside the card's 480px cap. Measure the rendered container before choosing the step.
- **Every cell in a `subgrid` row must render on every row, including the empty ones.** A conditional cell does not leave a hole where it was — the cells after it each shift one track left, so the rows that differ are exactly the rows that fall out of alignment. The follow-up queue's unassigned rows slid their completion control into the assignee column, moving the one control the reader aims at. Render a `hidden`-below-the-threshold placeholder instead.
- **Row meta must align into real columns.** A right-anchored `flex` cluster is only as wide as its own content, so `Enquiry`+`Overdue` and `Member`+`22 Feb 2027` start at different x and no two rows agree where the state column is — the queue cannot be scanned downward for what is late. Give the list the tracks (`@2xl/follow-ups:grid-cols-[fit-content(16rem)_minmax(0,1fr)_auto_auto_auto_auto] @2xl/follow-ups:gap-x-2.5`) and have each row re-enter them with `col-span-full grid grid-cols-subgrid`, the same way `lead-funnel.tsx` aligns its stage rows. Rules for the tracks: the identity column is `fit-content(<cap>)` so short names keep the flexible column close while one long name widens every row rather than only its own; meta columns stay `auto`, never fixed widths, because the due cell holds a badge or a **localized** medium date and only content sizing survives a change of account locale. Where a column is conditional (the follow-up queue's Enquiry/Member tag renders only under the All chip), declare a second template minus that track instead of leaving an empty one to pay for a gap. A cell whose content is optional still renders — `hidden … @2xl:block` when empty — or the row loses a column and its meta drifts out of line with its neighbours. Where there is no width to distribute, keep the wrapping flex line, give the promoted field `basis-full` so it takes its own line under the identity, and dissolve the mobile meta wrapper with `contents` so its children become real cells only where there is slack. **The switch is a container query, not a viewport breakpoint** — a queue sharing a row gets a ~560px column at a 1440px viewport, so `sm:` would lay six tracks into it and pay for them out of the note, the one field carrying content. Name the container on the card body (`@container/follow-ups`) and switch at the width the tracks actually need.
- **Splitting a shared cell into columns extends the component, it does not fork it.** `follow-up-task-summary.tsx` exports the stacked `FollowUpTaskSummary` for bounded table cells plus `FollowUpTaskLabel` and `FollowUpTaskNote` for row layouts that give the note a column. All three resolve the icon and heading through the same internal helpers, so the task vocabulary cannot drift between the dashboard queue and the Leads/Members tables.
- **Supporting text beside its heading matches the heading's size; only text beneath it may shrink.** The rule is why a note set beside its name cannot be demoted by size — at the name's own size it reads as a peer, and one step down it reads as smaller text rather than as hierarchy. A follow-up note is therefore always the **stacked** shape `FollowUpTaskSummary` renders: 12px muted under the 14px task heading, where size and tone share the demotion, capped at `max-w-56` so one long note cannot stretch its cell. The dashboard queue held the peer-sized alternative for one release and gave it up with the column it needed (see the queue-row rule above), which took the second `variant` with it. Do not reintroduce a size role through a call-site `className` — `className` on these components stays external layout.
...
A trigger whose primitive sets its own ARIA role, such as `Switch`, must also pass that `role` as a prop. A blocked trigger renders through the popover trigger, and its `role="button"` otherwise replaces the switch role while leaving `aria-checked` behind. Give a permission-blocked `Switch` `readOnly` too, so it announces as read-only and cannot change. Canonical: the rule switch in `renewal-reminders-settings.tsx`.

The explanation reads as an alert, not a dialog: it borrows the `Alert` grammar (`ui/alert.tsx`) — a `TriangleAlert` in `text-amber-foreground` in its own column, the title and reason beside it, the resolution left-aligned under the copy as an `outline` `size="sm"` button — and anchors itself to the blocked control with `PopoverArrow`, whose tail borders only the two edges facing away from the panel so the popover's hairline continues around the tip without crossing its base. A tailed popover must pass `POPOVER_ARROW_SIDE_OFFSET` as its `sideOffset`; the default 4px gap is narrower than the tail's reach. This panel is the one popover framed at 16px instead of the master's 10px, because it is read rather than picked from; that is a property of the blocker master, not a licence to reframe other popovers at a call site. Its corner dismiss control uses the shared `Primitive.Close` + ghost icon `Button` + `XIcon` recipe that Dialog and Sheet use, at `icon-xs`; outside press, Escape, and re-pressing the trigger still close it. It composes that grammar rather than nesting an `Alert`, whose card and `role="alert"` would box a panel inside a panel and re-announce a popup the popover already names. `PopoverArrow` is opt-in: no other popover grows a tail by default. `/preview/resolvable-action` is the dev-only harness for every blocker shape and anchor side.
...
**Every read-only phone render uses `fmt.phone(value)`.** It presents national numbers with the account code, adds `+` to legacy digits-only qualified values, and canonicalizes an international `00` prefix without changing persistence, search, `tel:` links, or clipboard values. `MemberIdentity` applies this automatically to its communication line (non-phone strings pass through), and phone-bearing CSV exports and notification fallbacks use the same formatter. Never render `contact.phone`, `contact_phone`, or a phone-typed custom field directly in visible copy. Invalid source text remains unchanged so display formatting cannot disguise a data-quality problem.
...
The header keeps Chat, Call, Note, and Email as compact icon actions. Users who can delete the lead also see Delete as the last icon action, with a `text-destructive` icon and label, a red border, and a red hover tint; it opens the shared confirmation dialog. **Add as member** is the full-width primary `Button` directly below the icon row, with its complete action label visible in both the enquiry sheet and the Chats contact panel. Do not pin a second Delete control below the scrollable details.
...
| `actions`           | allowlist of `ContactQuickActionId` — the inbox drops `chat` because you're already in the thread                                                                               |
...
Adding a lead field once surfaces it in the table, the sheet **and** the inbox. The inbox panel is fully editable (same writes, same RLS, same transfer/assignment approval RPCs) and re-pulls the page's `activeContact` via `onUpdated` so the thread header + conversation list can't go stale. **Company is deliberately not a gym lead field:** do not expose it in lead creation/editing, imports, preview, table/board views, bulk edit, profile details, inbox filters, or exports. The legacy database/API column remains compatibility-only.

The two overlays (`MemberForm` / `TransferRequestDialog`) live inside the content and therefore nest inside `SheetContent` on `/leads` — that's the established shape (member-detail nests its invoice/payment dialogs the same way), not a hazard. The lead header has one WhatsApp-marked **Chat** action: it resolves or creates the contact's canonical conversation and navigates to **Chats** (`/inbox`), where the composer owns both free-form and template sends.
...
`FollowUpTaskSummary` (`components/follow-ups/follow-up-task-summary.tsx`) is the canonical **task-led** follow-up cell: task-type icon + task label, with an optional note beneath. Enquiry queues use that base treatment; member queues additionally pass the member-only neutral Reason badge. Do not reconstruct it at a call-site.

`FollowUpTaskLine` from the same file is its **person-led** counterpart, and the choice between them is whose row it is. A follow-up table is a list of tasks, so the task leads. A queue row inside a list of people is a person — the dashboard's merged queue sits beside three other queues that all lead with `MemberIdentity`, and leading it with a task icon made it the one row on the page that did not look like its neighbours. There, `MemberIdentity` owns the identity and `FollowUpTaskLine` is its `meta`: one muted line carrying the task icon plus the note, falling back to the task label when there is no note so no row loses its second line. Both shapes read the same `TASK_ICON` map and `resolveTask`; a third shape is not the answer to a new layout.
...
- **Usual time** is the same optional planned time in All members and Attendance. **Check-in** and **Check-out** remain actual attendance timestamps; do not relabel either as the usual time.
...
- In enquiry and follow-up surfaces, the accountable task is always called **Follow-up**. Use **No follow-up** and **Add follow-up**; never expose “next action” as a synonym. “Task” is an internal word only — user-facing buttons and descriptions say **Cancel follow-up** / **Cancel follow-ups**, never “Cancel task”.
...
an upload staff already made, so those keep their shell and their own Send,
