...
- **Row meta must align into real columns.** A right-anchored `flex` cluster is only as wide as its own content, so `Lead`+`Overdue` and `Member`+`22 Feb 2027` start at different x and no two rows agree where the state column is — the queue cannot be scanned downward for what is late. Give the list the tracks (`sm:grid-cols-[fit-content(16rem)_minmax(0,1fr)_auto_auto_auto_auto] sm:gap-x-2.5`) and have each row re-enter them with `col-span-full grid grid-cols-subgrid`, the same way `lead-funnel.tsx` aligns its stage rows. Rules for the tracks: the identity column is `fit-content(<cap>)` so short names keep the flexible column close while one long name widens every row rather than only its own; meta columns stay `auto`, never fixed widths, because the due cell holds a badge or a **localized** medium date and only content sizing survives a change of account locale. Where a column is conditional (the follow-up queue's Lead/Member tag renders only under the All chip), declare a second template minus that track instead of leaving an empty one to pay for a gap. A cell whose content is optional still renders — `hidden … sm:block` when empty — or the row loses a column and its meta drifts out of line with its neighbours. Below `sm` there is no width to distribute: keep the wrapping flex line, give the promoted field `basis-full` so it takes its own line under the identity, and dissolve the mobile meta wrapper with `sm:contents` so its children become real cells only where there is slack.
...
- **Supporting text beside its heading matches the heading's size; only text beneath it may shrink.** `FollowUpTaskNote` names the two roles with `variant`. `subtitle` is the stacked cell — 12px under a 14px heading, where size and tone share the demotion. `column` is the row layout, where the note is a **peer** of the name rather than beneath it: a smaller size there does not read as hierarchy, it reads as smaller text, so the column note sits at the name's 14px and lets the muted tone carry the demotion alone. Verified at 5.79:1 (dark) and 5.52:1 (light) against the card, clearing the 4.5:1 body threshold that the promotion to body size makes binding. The `max-w-56` cap belongs to the stacked cell for the same reason and is passed in by that caller. Type role is a **prop**, never a `className` override at a call site — `className` on these components stays external layout.
