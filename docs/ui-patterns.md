...
- **Splitting a shared cell into columns extends the component, it does not fork it.** `follow-up-task-summary.tsx` exports the stacked `FollowUpTaskSummary` for bounded table cells plus `FollowUpTaskLabel` and `FollowUpTaskNote` for row layouts that give the note a column. All three resolve the icon and heading through the same internal helpers, so the task vocabulary cannot drift between the dashboard queue and the Leads/Members tables. The `max-w-56` note cap belongs to the stacked cell and is passed in by that caller — a note that owns a column must not carry a cap meant for a subtitle.
