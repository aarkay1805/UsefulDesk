...
- **Row meta must align into real columns.** A right-anchored `flex` cluster is only as wide as its own content, so `Lead`+`Overdue` and `Member`+`22 Feb 2027` start at different x and no two rows agree where the state column is — the queue cannot be scanned downward for what is late. Give the list the tracks (`@2xl/follow-ups:grid-cols-[fit-content(16rem)_minmax(0,1fr)_auto_auto_auto_auto] @2xl/follow-ups:gap-x-2.5`) and have each row re-enter them with `col-span-full grid grid-cols-subgrid`, the same way `lead-funnel.tsx` aligns its stage rows. Rules for the tracks: the identity column is `fit-content(<cap>)` so short names keep the flexible column close while one long name widens every row rather than only its own; meta columns stay `auto`, never fixed widths, because the due cell holds a badge or a **localized** medium date and only content sizing survives a change of account locale. Where a column is conditional (the follow-up queue's Lead/Member tag renders only under the All chip), declare a second template minus that track instead of leaving an empty one to pay for a gap. A cell whose content is optional still renders — `hidden … @2xl:block` when empty — or the row loses a column and its meta drifts out of line with its neighbours. Where there is no width to distribute, keep the wrapping flex line, give the promoted field `basis-full` so it takes its own line under the identity, and dissolve the mobile meta wrapper with `contents` so its children become real cells only where there is slack. **The switch is a container query, not a viewport breakpoint** — a queue sharing a row gets a ~560px column at a 1440px viewport, so `sm:` would lay six tracks into it and pay for them out of the note, the one field carrying content. Name the container on the card body (`@container/follow-ups`) and switch at the width the tracks actually need.
...
`FollowUpTaskSummary` (`components/follow-ups/follow-up-task-summary.tsx`) is the canonical **task-led** follow-up cell: task-type icon + task label, with an optional note beneath. Lead queues use that base treatment; member queues additionally pass the member-only neutral Reason badge. Do not reconstruct it at a call-site.
