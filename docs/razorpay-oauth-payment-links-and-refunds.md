...
- **Legacy-row classification:** the selected Live merchant's API-key surface shows **Generate Key**, not an existing key. Therefore the production database's old manual credential row cannot be a Live credential; this is provider evidence, not a key-prefix inference. Do not migrate it as `live`, use it for rollback, or copy it into Live configuration. Preserve it blocked for explicit cleanup until a new Live rollback credential is intentionally generated after the OAuth-rotation gate.
- **Credential-rotation blocker:** both OAuth clients' secrets were visible during acceptance and must rotate before Live authorization. Razorpay application settings expose each as a disabled reveal-only field and provide no rotate/regenerate control; the official OAuth integration steps document consumption but no rotation endpoint. Do not delete/recreate the application or silently retain the exposed secrets. Obtain a provider-supported rotation for both client secrets, then rotate the Test/live application webhook secrets, Test merchant webhook secret, isolated Test service-role key, and only the exact remaining acceptance-exposed set. Until that succeeds, do not create the live webhook, add production secrets, apply production migrations, generate live rollback keys, authorize OAuth, or perform API/refund/money exercises. Stage 5 remains unaccepted.
- **Rotation request evidence (2026-08-10):** Razorpay support ticket `20303463` requests rotation of both the Development and Production OAuth client secrets while preserving the application, client IDs, redirect URIs, and webhook configuration. The request contains no current secret, token, webhook secret, credential value, or member data. Razorpay reports a 4–8 business-hour update window. This ticket is pending evidence only: it does not satisfy rotation, authorize a Live merchant, or permit production configuration, migration, webhook, API, refund, or real-money work.
