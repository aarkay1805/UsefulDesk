...
**Status:** Razorpay Technology Partner account active and partner onboarding completed on 2026-08-08. Isolated development OAuth/API/product acceptance and Stages 1–4 passed for the single Test account, and owner-controlled Stage 5 Live provider/payment acceptance passed on production account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` / merchant `acc_TCJwBqanN9LTrK`. Current OAuth readiness was restored on 2026-08-11 and revalidated under separate owner authority on 2026-08-14 through a new exact-account/merchant-pinned recovery path: the stored grant rotated to generation 2, provider readiness became fresh, and the closeout produced zero operational queues or new financial/messaging records. The owner has since confirmed this is their operating gym account with real clients, so that exact connection-readiness revalidation is the first real gym-owner OAuth readiness pilot. On 2026-08-12 the owner explicitly waived the remaining policy-only 14-day rollback hold and Stage 6 retired manual keys: connector-applied migration `20260811181302_retire_razorpay_manual_keys.sql` locks Test and Production to OAuth/storage-v1/application ingress, erased Test's one dormant legacy webhook secret, and requires all manual columns null. Manual UI/API/config, Basic-auth and version-0 compatibility, legacy per-account ingress, and cutover RPCs are removed while immutable audit evidence remains. After repairing Production's template dry-run mode and Meta's trailing-parameter rejection, an authenticated 2026-08-15 sync proved `gym_payment_link` (`1996323644342719`) and `gym_payment_due` (`1528972491789269`) **Approved**. Under separate exact owner authority, READY Production deployment `dpl_C6dFzbhXXjZ3ZULLq7uCbZLJp4mW` opened the shortest OAuth window, created one full-balance ₹40 Live Payment Link for invoice `#BC2B1DDB`, and sent the approved template through the existing conversation; it advanced to **Read**. Signed application event `TPy00PfdmPIwtD` then created one immutable ₹40 Payment Link payment and one ₹40 allocation, reducing the invoice's collectible/accounting balances to zero with no pilot refund or review hold. Exact Live operational queues remained zero. READY deployment `dpl_DAtth8pTbH8osaCiSVao71wpoi5x` restored OAuth false while first-bind enrollment and every provider/refund acceptance flag remained false. VBF/Aakash remains closed. Existing OAuth client secrets remain unrotated under the explicit owner risk acceptance.
...
**Rollout:** Development-client sandbox acceptance, the owner-controlled Live gate, the Rajat real gym-owner connection-readiness pilot, and one separately authorized real-client ₹40 Payment Link delivery/settlement passed. Production remains pinned only to Rajat's accepted account/merchant with OAuth, enrollment, and acceptance flags disabled at rest. The explicit owner risk decision superseded client-secret rotation as a Stage 5 prerequisite; the secrets remain unrotated. No fleet-wide rollout, second customer exercise, or refund is authorized.
