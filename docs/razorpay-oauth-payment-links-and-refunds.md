...
**Status:** Razorpay Technology Partner onboarding, isolated Test Stages 1–4, owner-controlled Live Stage 5 acceptance, and Stage 6 OAuth-only retirement are complete. Production is permanently operational only for UsefulDesk account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` / exact Live merchant `acc_TCJwBqanN9LTrK`: `RAZORPAY_OAUTH_ENABLED=true` is the ongoing operating state behind those existing pins, while first-bind enrollment and every provider/refund acceptance flag remain false. READY deployment `dpl_9dcvUKMuTMiXzw8xsC21GQ49cfhp`, built from commit `26149600cdbe08c028736e8074761505199ecb72`, is promoted and aliased to `desk.usefulmade.com`. Authenticated UI verification shows Connected, Readiness verified, Live mode, the expected merchant suffix, and enabled payment-link actions on an already-existing eligible invoice without creating or sending a link. The secret-blind closeout found the OAuth/Live/storage-v1/application/`read_write`/ready generation-2 connection unchanged, no manual material, lease, or error, zero relevant queues, and zero new financial, messaging, refund, link, mandate, webhook, or OAuth-state records. The first real-client ₹40 settlement remains exactly once with no refund. VBF/Aakash remains closed, and existing OAuth client secrets remain unrotated under the explicit owner risk acceptance.
...
**Rollout:** Development-client sandbox acceptance, the owner-controlled Live gate, the Rajat real gym-owner connection-readiness pilot, and one separately authorized real-client ₹40 Payment Link delivery/settlement passed. Production remains pinned only to Rajat's accepted account/merchant with OAuth permanently enabled; enrollment and all acceptance flags remain disabled. The explicit owner risk decision superseded client-secret rotation as a Stage 5 prerequisite; the secrets remain unrotated. No fleet-wide rollout, first binding for another gym, second customer exercise, WhatsApp send, or refund is authorized.
...
- Template approval gates **Send payment link** only. **Copy link**, OAuth, Payment Link creation, and settlement proceed through their own acceptance and action-authorization gates.
...
- Keep OAuth disabled for any account without an explicit exact account/merchant operating authorization. Production's sole exception is the permanently pinned Rajat tuple recorded in Stage 6; enabling OAuth does not enable first-bind enrollment, and revoked/blocked OAuth never falls back because manual rollback was retired.
...
- **WhatsApp/template status at Stage 3 acceptance:** `gym_payment_link` was not approved, so the UI correctly disabled Send with setup guidance. Copy, provider creation, verified settlement, cancellation, revision, and reconciliation remained available and passed. The later 2026-08-15 Production approval does not fabricate Send evidence for this historical Test exercise.
...
- **Accounting and containment evidence:** Finance shows ₹1 gross, ₹1 refund, ₹0 net, one open invoice, ₹1 outstanding, and zero refund review. The downloaded August invoice CSV contains the exact payment/refund ids, `reopen_balance`, gross/refund/net `1/1/0`, and collectible balance `1`. Mohit's separate ₹2,700 joining invoice, membership due, and reminder row stayed unchanged; the sale refund did not enter membership dues/reminders. `gym_payment_link` was unapproved during this exercise and WhatsApp Send was not exercised; its later approval does not change that evidence. The single-use catalog item is archived while its immutable sale/invoice history remains.
...
- **Permanent exact-tuple activation (2026-08-15):** the owner authorized ongoing Production OAuth operation only for account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` and Live merchant `acc_TCJwBqanN9LTrK`. A secret-blind preflight proved the exact encrypted generation-2 OAuth/Live/storage-v1/application/`read_write`/ready binding, no manual material, active state, lease, error, Test/Live crossover, conflicting merchant, or relevant queue, plus the approved `gym_payment_link` template. The only environment mutation set `RAZORPAY_OAUTH_ENABLED=true`; first-bind enrollment and every provider/refund acceptance flag stayed false, the exact pins stayed unchanged, and the retired manual rollback variable remained absent. READY deployment `dpl_9dcvUKMuTMiXzw8xsC21GQ49cfhp` from commit `26149600cdbe08c028736e8074761505199ecb72` is promoted on `desk.usefulmade.com` with no observed error logs. Authenticated UI verification showed Connected, Readiness verified, Live mode, the expected merchant suffix, and enabled Copy/Send controls on an existing eligible invoice; neither action was used. The closeout at `2026-08-15T08:12:04.581065Z` found every relevant queue at zero, the prior ₹40 payment/allocation and invoice settlement unchanged, and zero new invoices, lines, payments, allocations, refunds, adjustments, links, messages, conversations, Razorpay events, mandates, or OAuth states. No consent, link, WhatsApp send, transaction, refund, money movement, secret rotation, VBF action, or broader rollout occurred.
