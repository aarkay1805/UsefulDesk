...
- **Current gate:** dual-ingress parity and OAuth Bearer AutoPay mutation acceptance are complete. Do not change the selector yet: the application route is still deliberately observation-only and fails closed if selected. Implement and verify the shared recovery/due-token scan and canonical application processor first, then perform the guarded Test-account cutover and prove post-cutover legacy observation with zero canonical claim plus application replay/idempotency. Synthetic Test objects must be cancelled/cleaned where Razorpay permits, and every acceptance credential must rotate again before any live merchant authorises.
