...
- **Current real-money stop gate:** final READY deployment `dpl_8ur1o5njFRfm66Qu8X88cY6svL6U` serves `desk.usefulmade.com` with OAuth, manual rollback, provider acceptance, and refund acceptance flags false; invalid application-webhook signatures return 400. No Live payment, refund, signed provider delivery, duplicate/replay, Finance mutation, reminder, CSV change, or WhatsApp Send evidence has been claimed. Before any money moves, explicitly name the payer, reconfirm merchant `acc_TCJwBqanN9LTrK`, choose the low-value amount, and choose `reopen_balance` or `reduce_charge`. The unapproved `gym_payment_link` template keeps WhatsApp Send out of scope. Stage 5 remains unaccepted.
