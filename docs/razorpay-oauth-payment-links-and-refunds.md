...
**Status:** Razorpay Technology Partner account active and partner onboarding completed on 2026-08-08. Isolated development OAuth/API/product acceptance and Stages 1–3 passed for the single Test account: Stage 2 has exact parity, durable recovery/token scanning, one canonical processor, atomic application cutover, and genuine provider retry; Stage 3 has exact full-balance Payment Link creation/reuse, cancellation/revision, paired signed settlement, and exact-once allocations. The missing `gym_payment_link` template still disables Send only. Production/manual-account inventory, broader rollout, credential rotation, and all refund work remain separate authority.
...
- **Implemented and applied only in UsefulDesk Razorpay Test:** migrations `20260809165718_razorpay_full_refunds.sql`, `20260809171336_harden_razorpay_full_refunds.sql`, `20260809171510_complete_razorpay_refund_recovery_contract.sql`, and `20260809172816_finance_refund_reporting.sql` add service-only refund reservation/finalization/import/classification boundaries, immutable copied allocations, append-only adjustments, recovery/cursors, exception containment, RLS, and refund-aware reporting. The admin API/UI uses the named `canRefundGatewayPayments` predicate; no shared UI master changed.
...
- **External partial containment passed:** Razorpay Dashboard Test Mode created ₹1.00 refund `rfnd_TNlC69tk2RY9yk` against a ₹1.01 local payment. Signed event `TNlCjoMScyDhIs` imported a header-only processed refund with no disposition/allocation, reduced net cash to ₹0.01, set collectible balance to zero despite ₹1.00 line accounting balance, disabled unsafe actions, and exposed one `partial_refund_line_target_required` exception. No line allocation was invented and this release intentionally provides no classification/clear action.
...
- **Acceptance status — not accepted under the stricter task gate:** all canonical Razorpay events are processed and no refund remains `creating`, `pending`, or `orphaned`, but the required external-partial exercise deliberately leaves the one unresolved line-targeting exception above. Razorpay also delivered the Stage 4 refund event only once, so the prior Stage 2 genuine provider-retry evidence covers the shared canonical claim path but is not a Stage 4 refund redelivery. Do not describe Stage 4 as accepted until the required zero-unresolved-exception condition is reconciled with the deliberately blocked partial-refund requirement and genuine refund duplicate/replay evidence is obtained.
- **Restored state:** READY deployment `dpl_JB2XKxvf7MBqUJ53u9ajMSaDJisF` restored `RAZORPAY_OAUTH_ENABLED=false` and `RAZORPAY_REFUND_AMBIGUOUS_CREATE_ACCEPTANCE=false`; manual rollback remains false and mode remains Test. No other account, production/Live Mode, real money, credential rotation, legacy retirement, or Stage 5 work is authorized.
