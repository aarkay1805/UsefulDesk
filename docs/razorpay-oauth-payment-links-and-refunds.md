...
**Status:** Razorpay Technology Partner account active and partner onboarding completed on 2026-08-08. Isolated development OAuth/API/product acceptance and Stages 1–4 passed for the single Test account: Stage 2 has exact parity, durable recovery/token scanning, one canonical processor, atomic application cutover, and genuine provider retry; Stage 3 has exact full-balance Payment Link creation/reuse, cancellation/revision, paired signed settlement, and exact-once allocations; Stage 4 has full remaining-payment refunds, both accounting dispositions, historical Dashboard import, ambiguous-create recovery, genuine provider retry, and explicit line-targeted resolution of an external partial refund with zero unresolved events/exceptions. The missing `gym_payment_link` template still disables Send only. Stage 5 selected production branch `50a9e8f9-d7e5-44d2-ba04-c367509b981e` and activated, untransacted Live merchant `acc_TCJwBqanN9LTrK`; Live Payment Links, Subscriptions, and the exact consumed event selector are available. Production now has fail-closed Live configuration, Stage 1–4 schema, audited Live-selector gate, legacy non-Live containment, sensitive Production OAuth credentials, and one exact 16-event Live application webhook. The owner explicitly accepted temporary use of the existing unrotated OAuth client secrets. OAuth authorization stopped before Connect because the active UsefulDesk browser session belonged to a different workspace; the flag is restored false and no provider grant, connection, signed delivery, API probe, or financial mutation exists. Correct UsefulDesk pilot authentication is the current gate; Stage 5 is not accepted.
...
- **Current stop gate:** the credentials and exact Live webhook are complete. A shortest OAuth window deployed `RAZORPAY_OAUTH_ENABLED=true` on `dpl_CTFvW7oSpUuMkM4XVZ3qyMnskb3s`, but the active UsefulDesk browser session belonged to workspace **Aakash Mishra**, not the allowlisted Rajat Kashyap pilot; the exact pilot branch URL rendered **Branch unavailable**. No Connect action, OAuth state, provider authorization, connection, capability probe, selector activation, signed delivery, reconciliation, or financial mutation occurred. The flag was immediately restored to `false` on `dpl_CJuMiLV5EW5VhYpCogGcK6YS248v`. Rajat must sign in to the UsefulDesk workspace with account id `50a9e8f9-d7e5-44d2-ba04-c367509b981e` before the shortest OAuth exercise is repeated. After OAuth readiness, separately obtain an explicit payer, reconfirm merchant `acc_TCJwBqanN9LTrK`, choose the low-value amount, and choose the refund disposition before any real-money action. Stage 5 remains unaccepted.
