...
**Status:** Razorpay Technology Partner account active and partner onboarding completed on 2026-08-08. Isolated development OAuth/API/product acceptance and Stage 1 passed, and Stage 2 now has exact three-event parity, durable recovery/token scanning, one shared canonical processor, an authenticated Test-only atomic cutover, and a fresh application-canonical/legacy-shadow cancellation. Razorpay does not expose immediate replay of a successful event: a Technical Support replay is the remaining Stage 2 acceptance gate. Production/manual-account inventory and credential rotation remain separate pre-live authority. Refund accounting requires the separate full-refund stage below.
...
- **Current gate:** selector cutover, fresh post-cutover dual observation, exactly-once canonical processing, and mutable Subscription cleanup are complete. Razorpay's documented replay path for a successful event requires a Technical Support request; no immediate dashboard replay control exists, and reserializing the stored JSON would violate the raw-body/signature contract. Do not call Stage 2 accepted, cut over another account, or remove legacy until Razorpay replays the successful Test cancellation and the same event remains processed with `attempt_count=1`, no second mutation, no unresolved event, and both observations intact. Every credential visible during acceptance—including development and production OAuth client secrets—must rotate before any live merchant authorises.
