...
**Status:** Razorpay Technology Partner onboarding, isolated Test Stages 1–4, owner-controlled Live Stage 5 acceptance, Stage 6 OAuth-only retirement, and the database-owned multi-account rollout gate are complete. Rajat account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` remains exactly bound to Live merchant `acc_TCJwBqanN9LTrK`; VBF account `9c50dcd9-ed4a-427c-a2fc-07d452f0aec7` is the sole unbound first-bind canary. Browser roles cannot read or mutate rollout authority. VBF has no credential, binding, or active OAuth state, so G12 still awaits the application release, VBF-owner Live consent, and the no-money readiness/isolation/zero-queue closeout. Existing OAuth client secrets remain static under Razorpay's confirmed provider constraint.
...
**Rollout:** Development-client sandbox acceptance, the owner-controlled Live gate, the Rajat real gym-owner connection-readiness pilot, and one separately authorized real-client ₹40 Payment Link delivery/settlement passed. Production rollout authority now contains Rajat as bound and VBF as the only first-bind account; the old account/merchant environment pins are retired by the application release containing this entry. No fleet-wide rollout, VBF transaction, second customer exercise, WhatsApp send, or refund is authorized.
...
When the application ingress is active, unknown but correctly signed external account IDs remain in `webhook_events` with `account_id = NULL`, return 2xx to avoid futile retries, and alert operators. They remain failed/unprocessed rather than being discarded; the recovery worker may bind them only after the exact OAuth external merchant + provider mode exists, then uses the normal owner-leased canonical processor. During shadow mode they remain only in `razorpay_webhook_deliveries`, where they still raise the routing-parity alert but cannot create canonical state.
