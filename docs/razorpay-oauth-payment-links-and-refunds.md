...
- **Implemented in code (2026-08-09):** the legacy route records `legacy_account` observations before consulting the selector and remains the only canonical claimant. It stays signature-verifying after cutover but returns after observation. Header event ids and mode/merchant/raw-body fallback hashes are shared across ingresses.
- **Implemented in code (2026-08-09):** the application route verifies the current or previous application secret, resolves `(RAZORPAY_MODE, account_id)`, durably records only `application` shadow observations, and performs no `webhook_events`, handler, payment, mandate, refund, or exception mutation. If an operator prematurely selects application canonical ingress, the route returns 503 rather than acknowledging an unprocessed money event.
...
- **Current gate:** the ledger is empty and no real legacy/application pair has been delivered. The shadow code is READY on isolated deployment `dpl_9eqaEeW4F8saCnwdgvXs9r9BS7n2` with both rollout flags false. A synthetic request signed from the CLI-pulled Vercel environment returned `Invalid signature`; no row was written. Do not guess or overwrite this secret—review the Razorpay application-webhook and Vercel secret sources through authenticated operator access, then configure a test-merchant legacy webhook secret/endpoint in Razorpay Dashboard. Reconnect the isolated OAuth merchant, generate one attributable test event, and require equal event id, external/resolved account, type, hash, and acceptable arrival skew with zero shadow mutation attempts. OAuth Bearer AutoPay mutation validation, the shared recovery/due-token scan, canonical application processor, and guarded cutover must not proceed until that evidence exists.
