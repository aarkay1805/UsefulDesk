...
- **Acceptance status — not accepted under the stricter task gate:** all canonical Razorpay events are processed, the refund-specific genuine duplicate/retry requirement is now satisfied, and no refund remains `creating`, `pending`, or `orphaned`. The required external-partial exercise deliberately leaves one unresolved `partial_refund_line_target_required` exception. Do not describe Stage 4 as accepted until the strict zero-unresolved-exception condition is reconciled with the deliberately blocked partial-refund requirement; do not clear or fabricate allocations for that exception.
- **Restored state:** READY deployment `dpl_7s1VtoUyXyUb3V2JiPqTCyovxsRj` restored `RAZORPAY_OAUTH_ENABLED=false`, `RAZORPAY_MANUAL_ROLLBACK_ENABLED=false`, `RAZORPAY_REFUND_AMBIGUOUS_CREATE_ACCEPTANCE=false`, and `RAZORPAY_REFUND_WEBHOOK_RETRY_ACCEPTANCE=false`; mode remains Test. No other account, production/Live Mode, real money, credential rotation, legacy retirement, or Stage 5 work is authorized.
