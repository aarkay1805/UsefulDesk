...
- **Verified 2026-08-12:** authenticated **Sync from Meta** succeeded for the current Production WABA and returned zero provider templates; no local `gym_payment_link` row exists. The owner must approve the exact language and body wording before submission. The category, name, and parameter order remain locked by section 10.2.
