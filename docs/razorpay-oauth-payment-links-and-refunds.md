...
**Status:** Razorpay Technology Partner account active and partner onboarding completed on 2026-08-08. Isolated development OAuth/API/product acceptance and Stages 1–4 passed for the single Test account, including genuine provider retries and zero unresolved events/exceptions. Stage 5 provider/payment acceptance passed on owner-controlled production account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` and exact Live merchant `acc_TCJwBqanN9LTrK`: the hardened `read_write` OAuth connection, exact 16-event application webhook, one ₹1 Live Payment Link/captured UPI payment, provider-backed historical refund scan, full `reopen_balance` refund, signed settlement/refund processing, exact allocations, Finance/invoice/dues/CSV checks, and final zero-queue checks all passed. This is not evidence of a real gym-owner rollout. The VBF/Aakash co-branded continuation is closed: four short-lived state reservations expired unconsumed, and VBF has no active state, credential, merchant binding, selector activation, Payment Link, gateway payment, or refund. The provider account-creation path and disabled-by-default first-bind code remain available but dormant. The missing `gym_payment_link` template still disables Send, and no WhatsApp Send evidence is claimed. The owner explicitly accepted temporary use of the existing unrotated OAuth client secrets; they are not described as rotated. READY deployment `dpl_J9UvqQCnTapi33A3zpGaEqCyP7Qx` repins Production only to Rajat's accepted account/merchant with OAuth, first-bind enrollment, rollback, and every acceptance flag false. A failed earlier provider revocation left the mutable Rajat connection status `disconnecting`; the merchant binding and encrypted current-dated token fields remain, but no current operational-readiness claim or money action is permitted until separate secret-blind provider reconciliation. No real-customer rollout or Stage 6 work is authorized.
