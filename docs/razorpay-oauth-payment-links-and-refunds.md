...
**Status:** Razorpay Technology Partner account active and partner onboarding completed on 2026-08-08. Isolated development OAuth/API/product acceptance and Stages 1–4 passed for the single Test account, including genuine provider retries and zero unresolved events/exceptions. Stage 5 provider/payment acceptance passed on owner-controlled production account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` and exact Live merchant `acc_TCJwBqanN9LTrK`: the hardened `read_write` OAuth connection, exact 16-event application webhook, one ₹1 Live Payment Link/captured UPI payment, provider-backed historical refund scan, full `reopen_balance` refund, signed settlement/refund processing, exact allocations, Finance/invoice/dues/CSV checks, and final zero-queue checks all passed. This is not evidence of a real gym-owner rollout. VBF account `9c50dcd9-ed4a-427c-a2fc-07d452f0aec7`, owned by Aakash Mishra, is now staged as the first real-gym co-branded signup pilot. The provider application exposes account creation, and the callback's disabled-by-default first-bind path cannot replace an existing merchant or infer activation from generic API access. Aakash has no Razorpay merchant or local credential row yet. The missing `gym_payment_link` template still disables Send, and no WhatsApp Send evidence is claimed. The owner explicitly accepted temporary use of the existing unrotated OAuth client secrets; they are not described as rotated. READY deployment `dpl_2cUJiNwwMQ26fNEjH4k79PwU7AcG` opens the VBF-only enrollment window with OAuth and first-bind enrollment true while rollback and acceptance flags remain false; signup, callback, merchant binding, and readiness are not yet claimed. Stage 6 and rollout beyond VBF remain deferred.
...
- **Co-branded real-gym pilot preparation (2026-08-10):** the earlier Stage 5 account belongs to the UsefulDesk owner and is reclassified as owner-controlled Live acceptance, not a customer merchant. Production VBF account `9c50dcd9-ed4a-427c-a2fc-07d452f0aec7`, whose authoritative membership makes Aakash Mishra owner, is selected as the sole real-gym pilot and has no `account_payment_credentials` row. The existing Razorpay application's Onboarding UI Configurator visibly offers **Create your account to get started**, matching Razorpay's standard co-branded login-or-signup flow; no second application is needed. The callback now accepts a returned merchant mismatch only while `RAZORPAY_LIVE_PILOT_ENROLLMENT_ENABLED=true`, still requires the exact UsefulDesk account allowlist and bound OAuth state/user/client/mode/redirect, requires no existing merchant binding, revokes rejected grants, and disables imported-account capability fallback for first-bind readiness. Deployment `dpl_4LtRKnJ87qypvh549sGGs1e4oWLV` is READY with `RAZORPAY_OAUTH_ENABLED=false`, `RAZORPAY_LIVE_PILOT_ENROLLMENT_ENABLED=false`, and every rollback/acceptance flag false. The production account allowlist is staged to VBF; no Aakash OAuth state, Razorpay account, KYC data, authorization, selector activation, merchant pin, or money evidence exists yet.
- **VBF enrollment window opened (2026-08-11):** after Aakash signed into the exact VBF workspace, Vercel Production variables `RAZORPAY_OAUTH_ENABLED` and `RAZORPAY_LIVE_PILOT_ENROLLMENT_ENABLED` were set true and deployment `dpl_2cUJiNwwMQ26fNEjH4k79PwU7AcG` reached READY on `desk.usefulmade.com`. The account allowlist remains exact to VBF and every manual/provider/refund acceptance flag remains false. This is configuration evidence only; Aakash must retry Connect Razorpay, and no OAuth state, provider merchant, KYC, authorization, selector activation, or money movement is claimed yet. Close enrollment and pin the returned merchant immediately after the callback.
...
- Co-branded Razorpay onboarding beyond the single VBF first-bind pilot
