...
- **Current gate:** dual-ingress parity, OAuth Bearer AutoPay mutation, and shared recovery/due-token scan acceptance are complete. Do not change the selector yet: the application route is still deliberately observation-only and fails closed if selected. Enable the shared canonical processor at the application ingress and add the parity-gated Test-only atomic selector transition first, then prove post-cutover legacy observation with zero canonical claim plus application replay/idempotency. Synthetic Test objects must be cancelled/cleaned where Razorpay permits, and every acceptance credential must rotate again before any live merchant authorises.
