...
`payment_allocations` and `invoice_credit_allocations` are append-only. Both allocate proportionally across open line balances and use deterministic largest-remainder paise rounding so allocations reconcile exactly. Member credit is branch/member-specific, non-transferable, non-expiring, and consumed oldest-first by the next manual checkout; AutoPay invoices never consume it.
