...
**Stage 5 production gate (2026-08-10):** read-only preflight found the production deployment has no `RAZORPAY_*` environment configuration and the isolated production database has not received the Stage 1–4 OAuth/link/refund migrations. One production account has a configured legacy manual key/webhook row, but the schema cannot record a reviewed provider mode or external merchant id, so it is only a pilot candidate and must not be inferred as Live or selected. The Razorpay production OAuth client/callback exists, while the live application webhook and live product/event capabilities are unconfigured or unverified. No state changed; OAuth/manual rollback remain disabled wherever configured.

**Still open:** richer `payment.failed` handling (an immediate "auto-pay failed, pay manually" nudge instead of waiting for `subscription.halted`); explicit pilot branch/merchant selection plus reviewed legacy-row mode; production manual-secret backfill only after that review; exact acceptance-credential rotation authority/scope; live Payment Links, Subscriptions/Recurring, Payments/refunds, application-webhook event selection, and merchant-readiness evidence; an explicit low-value payer/amount/refund exercise; approval of `gym_payment_link` before WhatsApp Send; any additional application-ingress/account rollout or legacy retirement; UsefulDesk-initiated partial refunds; embedded onboarding remains optional and later.
