...
**Still open:** richer `payment.failed` handling (an immediate "auto-pay failed, pay manually" nudge instead of waiting for `subscription.halted`); production/manual-account inventory and backfill; Stage 2 successful-event provider replay before any additional application-ingress cutover or legacy retirement; all acceptance-credential rotation before live authorisation; embedded onboarding remains optional and later.
