...
- `POST /api/payments/razorpay/webhook` — application ingress → in Test, require `RAZORPAY_PROVIDER_ACCEPTANCE_ONLY=true`; in Live, require it to be false. Then verify current/previous raw-body HMAC, resolve exact mode + top-level merchant account, record the durable observation, and enter the same canonical claim/handler only when that UsefulDesk account's selector is `application`; unknown or legacy-selected merchants remain observation-only.
