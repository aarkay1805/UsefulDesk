...
**Co-branded first real-gym pilot staged (2026-08-10):** VBF account `9c50dcd9-ed4a-427c-a2fc-07d452f0aec7`, owned by Aakash Mishra, is the sole next production tenant. It has no Razorpay credential row or merchant account. The existing application exposes Razorpay's provider-hosted account-creation UI. Live first binding is now permitted only while a separate server flag is explicitly enabled, only for the exact account allowlist, and only when no merchant is already bound; rejected grants are revoked and first-bind readiness requires authoritative account activation. Deployment `dpl_4LtRKnJ87qypvh549sGGs1e4oWLV` keeps OAuth and enrollment false, so no signup or provider mutation has started.

**Still open:** Aakash's provider-hosted signup/KYC/authorization and exact returned-merchant pin; broader co-branded rollout beyond VBF; richer `payment.failed` handling; approval of `gym_payment_link` before WhatsApp Send; legacy-ingress retirement; Stage 6 manual-key-retirement work; UsefulDesk-initiated partial refunds; and WhatsApp Send.
