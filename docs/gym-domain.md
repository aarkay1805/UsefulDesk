...
**Co-branded enrollment closed (2026-08-11):** the VBF/Aakash continuation is no longer the production pilot. Its four short-lived OAuth state reservations expired unconsumed, and VBF has no active state, Razorpay credential or merchant binding, selector activation, Payment Link, gateway payment, or refund. The first-bind implementation remains disabled and dormant. Production is repinned only to Rajat's accepted owner-controlled account and exact merchant; this remains provider/payment-path evidence, not a real gym-owner rollout. A failed earlier provider revocation left that row `disconnecting`, so the historical acceptance remains valid but current payment readiness must be re-established through a separate secret-blind provider check before any later money action.
