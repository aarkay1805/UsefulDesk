...
**Stage 5 production gate (2026-08-10):** pilot account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` is explicitly selected and maps to activated Live merchant `acc_TCJwBqanN9LTrK`. Live Payment Links and Subscriptions are available, and the live application selector exposes the same 16 consumed events as Test. The Live merchant has no API key, so the production database's legacy manual row is provider-confirmed non-Live and must never be reused. Production still has no `RAZORPAY_*` deployment configuration, live application webhook, or Stage 1–4 OAuth/link/refund migrations. Razorpay's application settings expose OAuth client secrets only as reveal-only fields with no documented self-service rotation; support ticket `20303463` requests rotation of both OAuth clients and has a stated 4–8 business-hour update window. Mandatory provider-assisted rotation still blocks Live authorization and every later mutation. No credential/configuration or financial state changed; OAuth/manual rollback remain disabled wherever configured.

**Still open:** richer `payment.failed` handling (an immediate "auto-pay failed, pay manually" nudge instead of waiting for `subscription.halted`); Razorpay-assisted rotation of both OAuth client secrets plus the remaining acceptance-exposed credential set before authorization; production configuration/migrations and containment or removal of the non-Live legacy row; live OAuth Payments/refund API probes, application-webhook creation, signed delivery, and connection/reconciliation evidence; an explicit low-value payer/amount/refund exercise; approval of `gym_payment_link` before WhatsApp Send; any additional application-ingress/account rollout or legacy retirement; UsefulDesk-initiated partial refunds; embedded onboarding remains optional and later.
