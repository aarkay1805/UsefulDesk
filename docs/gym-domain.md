...
**Stage 5 production gate (2026-08-10):** pilot account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` is explicitly selected and maps to activated, untransacted Live merchant `acc_TCJwBqanN9LTrK`. Live Payment Links and Subscriptions are available, and the Live application selector exposes the exact 16 consumed events. The owner accepted the temporary risk of using the existing, unrotated OAuth secrets after confirming they appeared only in a private browser-tool transcript; ticket `20303463` was asked to close with no rotation or application change. Production deployment `dpl_CJuMiLV5EW5VhYpCogGcK6YS248v` now enforces Live mode, the exact redirect and pilot id, with every rollout/acceptance flag false. Sensitive Production OAuth credentials and a new sensitive application-webhook secret are deployed only to Production. Razorpay has one Live root webhook with the exact required three account, seven Subscription, four Payment Link, and two refund events; invalid signatures return 400. Stage 1–4 plus the readiness-gated Live selector migration are applied through the approved connector. The provider-confirmed non-Live legacy material was erased from the pilot row without reading it; no manual Live key was created. OAuth connect refuses unreviewed legacy secrets and any non-pilot Live account, while the callback can switch to application ingress only through the service-only audited readiness transaction. A temporary OAuth window stopped before Connect because the active UsefulDesk browser session belonged to a different workspace and the exact pilot branch rendered unavailable; the flag was immediately restored false with no OAuth state, provider grant, selector change, or money movement.

**Still open:** richer `payment.failed` handling (an immediate "auto-pay failed, pay manually" nudge instead of waiting for `subscription.halted`); correct UsefulDesk authentication for the Rajat Kashyap pilot, pilot OAuth authorization, five Live capability probes, signed delivery, connection/reconciliation evidence, and zero-queue verification; an explicit payer/merchant/low-value amount/refund-disposition exercise; approval of `gym_payment_link` before WhatsApp Send; any additional application-ingress/account rollout or legacy retirement; UsefulDesk-initiated partial refunds; embedded onboarding remains optional and later. Stage 5 is not accepted.
