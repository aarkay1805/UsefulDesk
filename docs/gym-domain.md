...
**Stage 5 production gate (2026-08-10):** pilot account `50a9e8f9-d7e5-44d2-ba04-c367509b981e` is explicitly selected and maps to activated, previously untransacted Live merchant `acc_TCJwBqanN9LTrK`. The owner accepted temporary use of the existing unrotated OAuth secrets; they remain secret-blind and ticket `20303463` was asked to close without action. Production has the exact 16-event Live root webhook, strict Live configuration, the Stage 1–4 schema, and service-only audited selector activation. The provider-confirmed non-Live legacy material was erased without being read and no manual Live key exists. The callback now pins both the UsefulDesk pilot account and exact returned Razorpay merchant, revoking exchanged tokens on a mismatch. The exact pilot completed `read_write` OAuth and all five non-mutating readiness probes; its encrypted connection is ready, lease-free, manual-material-free, and application-canonical with one immutable activation audit. Migration `20260810162750` keeps operator health mode/merchant scoped while preserving 16 older null-mode/null-merchant legacy events unchanged. The exact Live scope has zero events, failures, missing-ledger rows, exceptions, links, or refunds. READY deployment `dpl_8ur1o5njFRfm66Qu8X88cY6svL6U` restores every rollout/acceptance flag false; the UI shows Connected, Readiness verified, Live mode, and no attention alert, and invalid signatures return 400.

**Still open:** richer `payment.failed` handling (an immediate "auto-pay failed, pay manually" nudge instead of waiting for `subscription.halted`); an explicit payer, reconfirmed pilot merchant, low-value amount, and refund disposition; one Live Payment Link/capture/full remaining-payment refund; genuine signed delivery and duplicate/replay evidence where available; reconciliation plus Finance/invoice/dues/reminder/CSV verification; approval of `gym_payment_link` before WhatsApp Send; any additional application-ingress/account rollout or legacy retirement; UsefulDesk-initiated partial refunds; embedded onboarding remains optional and later. Stage 5 is not accepted.
