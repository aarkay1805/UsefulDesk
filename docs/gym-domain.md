...
**Still open:** richer `payment.failed` handling (an immediate "auto-pay failed, pay manually" nudge instead of waiting for `subscription.halted`); production/manual-account inventory and backfill; approval of `gym_payment_link` before WhatsApp Send; any additional application-ingress/account rollout or legacy retirement; Stage 4 acceptance closure (refund-specific genuine retry evidence is complete, but the required partial-refund exception remains intentionally unresolved under the strict zero-exception gate); explicit line-targeted partial-refund resolution; all acceptance-credential rotation before live authorisation; embedded onboarding remains optional and later.
