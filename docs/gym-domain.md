...
**WhatsApp template contracts:** proactive gym templates require positive scoped
consent. Utility account/payment updates use `whatsapp_account_updates`;
membership/service renewals and promotions use `whatsapp_marketing`. The exact
nine-template registry, provider payloads, categories, and parameter order live
only in `src/lib/whatsapp/template-contracts.ts`. Feature sends require the
exact Approved/synced provider row; submission and a returned message id do not
prove approval or delivery.
...
**Still open:** selecting and separately authorizing any real gym-owner pilot; broader co-branded rollout; richer `payment.failed` handling; Meta approval of the genuinely provider-submitted `en_US` Utility `gym_payment_link` template before WhatsApp Send (numeric ID `1996323644342719`, authenticated 2026-08-14 sync **Pending**); the separately submitted three-parameter `gym_payment_due` reminder (numeric ID `1528972491789269`) is also **Pending** and does not replace the four-parameter Payment Link contract; UsefulDesk-initiated partial refunds; and WhatsApp Send.
