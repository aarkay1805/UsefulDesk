...
**Still open:** richer `payment.failed` handling (an immediate "auto-pay failed, pay manually" nudge instead of waiting for `subscription.halted`); Stage 1 rollout gates (reviewed real-account zero-version-0 secret backfill, PKCE confirmation, and one internal development connection—the schema/grants are verified in the isolated test project); Stage 2 application-webhook parity/cutover; embedded onboarding remains optional and later.
